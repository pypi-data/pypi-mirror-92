...
class LSTMCRFTrainer(object):
    def __init__(self, bilstmcrf_model: model.LSTMCRF, input_vocab, input_path,
                 ...
                 val_path=None, epochs=5, optimizer=optim.Adam, n_iter=5, azureml_logger=None):
    ...
    def save_model(self, steps=None):
        ...
        if self.azureml_logger:
            locallog = 'cp %s %s' % ('PosTaggingPytorch/TakeBlipPosTagger/model.py', self.save_dir)
            logging.info('Since azureml_logger is true, try to execute : "{}"'.format(locallog))
            os.system('cp %s %s' % ('PosTaggingPytorch/TakeBlipPosTagger/model.py', self.save_dir))
            self.azureml_logger.register_pos_tagging_model(
