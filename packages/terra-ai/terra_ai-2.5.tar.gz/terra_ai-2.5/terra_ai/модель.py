...
def авторизация_LabStory():
  ...
  elif json_response.status_code == 422: # Если пришел 200-ый код ответа (все хорошо)
...
def показать_изменение_маски_MNIST(number):
  ...
  f, ax = plt.subplots(10,10,figsize=(16, 16))
  for i in range(10):
    for j in range(10):
...
def показать_изменение_маски_АВТО(num):
  f, ax = plt.subplots(8,8,figsize=(26, 16))
  for i in range(8):
    for j in range(8):
      ax[i,j].imshow(imgs[i*8+j][:,:,num], cmap='gray')
      ax[i,j].axis('off')
