...
class DefaultEmitter(Emitter):
    ...
    def reduce_entities(self, identities: typing.List[int]):
        for k in identities:
            entity = self.entity_map[k].reduce()
    ...
    def reduce_relationships(self, identities: typing.List[int]):
        for k in identities:
            relationship = self.rel_map[k].reduce()
    ...
    def __reduce_entities(self, executor: concurrent.futures.ThreadPoolExecutor):
        pending_identities = set(self.entity_map.keys())
        tasks = []

        for i in range(PARTITION_NUM):
            tasks.append(
                executor.submit(self.reduce_entities, [c for c in pending_identities if c % PARTITION_NUM == i]))
    ...
    def __reduce_relationships(self, executor: concurrent.futures.ThreadPoolExecutor):
        pending_identities = set(self.rel_map.keys())
        tasks = []

        for i in range(PARTITION_NUM):
            tasks.append(
                executor.submit(self.reduce_relationships, [c for c in pending_identities if c % PARTITION_NUM == i]))
    ...
    def close(self):
        with concurrent.futures.ThreadPoolExecutor(max_workers=PARTITION_NUM) as executor:
            self.__reduce_entities(executor)
            self.__reduce_relationships(executor)
