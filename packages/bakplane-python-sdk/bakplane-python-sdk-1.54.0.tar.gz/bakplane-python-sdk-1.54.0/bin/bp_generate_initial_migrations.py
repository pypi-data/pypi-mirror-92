...
class MigrationProvider(ABC):
    ...
    def create_table(self, name: str, schema: str, columns: typing.List[Column], primary_key_columns: typing.List[str],
                     **kwargs) -> Table:
        return Table(
            name,
            self.get_metadata(),
...
class RedshiftProvider(MigrationProvider):
    ...
    def create_table(self, name: str, schema: str, columns: typing.List[Column], primary_key_columns: typing.List[str],
        ...
        return super(RedshiftProvider, self).create_table(name, schema, columns, primary_key_columns,
                                                          redshift_distkey=dist_key_columns[0],
                                                          redshift_sortkey=['effective_start_dt', 'effective_end_dt',
                                                                            'knowledge_start_dt', 'knowledge_end_dt',
                                                                            'valid_start_dt', 'valid_end_dt']
                                                          )
...
def create_schema_if_needed(
        f_up, f_down, provider: MigrationProvider, doc: typing.Dict[str, typing.Any]
):
...
def create_table_if_needed(
    ...
    tbl = provider.create_table(
        ...
        schema,
        columns,
        ['knowledge_hash', 'payload_hash', 'knowledge_end_dt', 'effective_start_dt', 'valid_start_dt']
    )
...
def handle_resource_migrations(
    ...
    down_file = os.path.join(
        "migrations", r["code"], provider.get_name(), DOWN_TABLES_MIGRATION_NAME
    )
    ...
    click.echo(f"Table down  migrations successfully written to `{down_file}`.")
...
def generate_initial_migration_for_resource(
    ...
    tbl_migration_file_up, tbl_migration_file_down = handle_resource_migrations(provider, r, doc)
    ...
    if upload:
        ...
        client.upload_file(tbl_migration_file_up, bucket,
                           tbl_migration_dst_path + os.path.basename(tbl_migration_file_up))
        client.upload_file(tbl_migration_file_down, bucket,
