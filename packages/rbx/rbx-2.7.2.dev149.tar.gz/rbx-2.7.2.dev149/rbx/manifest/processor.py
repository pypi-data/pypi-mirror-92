...
from rbx.aws.s3 import S3Bucket
...
class AssetFetcher:
    def __init__(self, mode, bucket=None, region=None):
        """
        Setup the asset fetcher with the correct mode and, if needed, prepare a client.

        Parameters:
            mode (FetchMode):
                How the Assets should be fetched (AWS or HTTP).
            bucket (str):
                The bucket to use when retrieving from AWS.
            region (str):
                The region to use when retrieving from AWS.
        """
        self.base_url = None
        self.client = None
        self.mode = mode

        if mode is FetchMode.AWS:
    ...
    def get_manifest(self, url):
        ...
        """
        if self.mode is FetchMode.HTTP:
        ...
        elif self.mode is FetchMode.AWS:
        ...
        if decoded_image is None:
            raise FatalException(
                "Image has an invalid format: {}{}".format(
                    self.client.name + '/' if self.client else base_url,
                    url,
                ),
            )
