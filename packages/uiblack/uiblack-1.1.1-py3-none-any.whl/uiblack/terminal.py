        ...
        for index in range(len(self._contents_console) - 1, 0, -1):
            if self._title is None:
                ...
                if ((fixed_height - 3) - inverse_index) < 1:
                    break
            pad = " " * (
                fixed_width - self._len_printable(self._contents_console[index])
            )
            if self._len_printable(self._contents_console[index]) > fixed_width:
                result = f"{self._contents_console[index][0: fixed_width - 3]}..."
            else:
