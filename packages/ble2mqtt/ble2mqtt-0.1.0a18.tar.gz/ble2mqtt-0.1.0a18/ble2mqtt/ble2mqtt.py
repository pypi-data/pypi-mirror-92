...
class Ble2Mqtt:
    ...
    async def debug_tasks(self):
        while True:
            logger.info('================')
            for dev, task in self._device_manage_tasks.items():
                logger.info(f'{dev}: {task}')
            logger.info(f'Total tasks: {len(aio.Task.all_tasks())}')
            for t in aio.Task.all_tasks():
                logger.info(f'- {t}')
            logger.info('================')
            await aio.sleep(120)
    ...
    async def _run_device_tasks(self, mqtt_connection_fut: aio.Future) -> None:
        ...
        finished = await run_tasks_and_cancel_on_first_return(
            ...
            self._loop.create_task(self.debug_tasks()),
            *tasks,
        )
        for t in finished:
            t.result()
