...
class Solo(Process):
    ...
    def generic(self):
        ...
        for ctr in range(iterations):
            ...
            # Set fades as defined by the parameters
            newAudio = self.declick(newAudio, parameters["fades"])

            # Do we need to pad front and/or back with some silence?
            if "front_silence" in parameters:
                if "back_silence" in parameters:
                    fsil = random.randrange(parameters["front_silence"])
                    bsil = random.randrange(parameters["back_silence"])
            ...
            if "reps" in parameters:
                addAudio = newAudio
                reps = random.randrange(parameters["reps"][0], parameters["reps"][1])
