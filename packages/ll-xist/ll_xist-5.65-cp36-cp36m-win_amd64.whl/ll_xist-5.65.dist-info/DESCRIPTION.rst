...
* ``ll.ul4on`` now supports "persistent" objects, i.e. objects that can be
...
* In sisyphus jobs, when the value of the ``nextrun`` parameter was a
  ``datetime.timedelta`` object, it was interpreted relative to the
  start of the run, not relative to now. This has been fixed.

* The modules ``ll._xml_codec`` and ``ll.xist.sgmlop`` have been updated
  to directly support and produce ``393`` style unicode strings.
  (``ll._misc`` already supported them).
