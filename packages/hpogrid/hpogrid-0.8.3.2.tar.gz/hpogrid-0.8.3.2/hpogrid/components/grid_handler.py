...
class GridHandler():
    ...
    @staticmethod
    def submit(config_input:str, n_jobs=1, site=None, time=-1, search_points=None, mode='grid', extra=[]):
        config = GridHandler.preprocess_config(config_input)
        if mode == 'grid':
            GridHandler.submit_grid(config, n_jobs=n_jobs, site=site,
                                    time=time, search_points=search_points, extra=extra)
        elif mode == 'idds':
            GridHandler.submit_idds(config, n_jobs=n_jobs, site=site, extra=extra)
        else:
    ...
    @staticmethod
    def submit_idds(config, n_jobs=1, site=None, extra=[]):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            with helper.cd(project_path):
                ...
                tatble = stylus.create_table(idds_config, ['Attribute', 'Value'], indexed=False)
                print(table)
                # submit jobs
                ...
                for _ in range(n_jobs):
                    os.system(command)
    @staticmethod
    def submit_grid(config, n_jobs=1, site=None, time=-1, search_points=None, extra=[]):
        project_path = helper.get_project_path(config['project_name'])
