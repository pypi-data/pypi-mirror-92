...
import Utils.SocketApiServe  as socketApiServe
...
def AddProps(obj, Props, args):
    ...
    if obj._HashCode is None:
        raise Exception("Missing unique key value:_HashCode!")
    elif obj._HashCode is not None:
        #Props = Props
...
#objkey:目标key
#default:找不到时返回的默认值
def dict_set(dict, objkey, value):
    tmp = dict
    if(tmp is None):
        return None
    for k,v in tmp.items():
        if k == objkey:
            ...
            if type(v).__name__=="dict":
                return dict_get(v, objkey, value)
    return False
