...
class Sem(AsdictLarge):
    atlas: int = 499
    atlas_new: int = round(atlas / sem_new_factor)
    db: int = 500
    io: int = 500
    max: int = 5000
    nmap: int = 500
    nmap_new: int = round(nmap / sem_new_factor)
    os: int = 300
    os_new: int = round(os / sem_new_factor)
    ping: int = 400
    ping_new: int = round(ping / sem_new_factor)
    socket: int = 500
    ssh: int = 300
    ssh_new: int = round(ssh / sem_new_factor)
    _factor: int = 1
...
async def aiocmd(command: UnionStrList, onlyout: bool = True,
                 onlyrc: bool = False, decode: bool = True, utf8: bool = False) -> Union[TupleStrStrInt, str, int]:
    """
    Asyncio run cmd.
...
def cmd(command: Iterable, onlyout: bool = False, onlyrc: bool = False, shell: bool = True, sysexec: bool = False,
        lines: bool = True) -> Union[TupleStrStrInt, int]:
    """
    Runs a cmd.
...
class Env(Asdict):
    ...
    def sem(self):
        s = [self.file.int(upper_prefix(k, prefix=self.prefix), v) for k, v in
             Sem(_factor=self.sem_factor).defaults().items() if k not in ['sem_factor', 'value']]
        return Sem(*s)
...
class MongoBase:
    ...
    Naps = collections.namedtuple('Naps', 'mongo os queue')
    naps = Naps(10, (0.1, 2.1, ), 2)
    _aclient: MotorClient = dataclasses.field(default=None, init=False)
    _client: MongoClient = dataclasses.field(default=None, init=False)
    ...
    def client(self, a: bool = False) -> Union[MotorClient, MongoClient]:
        while True:
            try:
                if a:
                    self._aclient = MotorClient(self.uri, connectTimeoutMS=200000, serverSelectionTimeoutMS=300000,
    ...
    def get_sleep(self, mongo: bool = True, queue: bool = False) -> Union[int, float]:
        if mongo:
            sleep = self.naps.mongo
        elif queue:
            sleep = self.naps.queue
        else:
            sleep = round(random.uniform(self.naps.os[0], self.naps.os[1]), 2)
        return sleep

    def nap(self, mongo: bool = True, queue: bool = False) -> None:
        time.sleep(self.get_sleep(mongo=mongo, queue=queue))

    async def anap(self, mongo: bool = True, queue: bool = False) -> None:
        await asyncio.sleep(self.get_sleep(mongo=mongo, queue=queue))
...
class Mongo(MongoBase):
    ...
    async def dump_async(self) -> list:
        await self.log.ainfo('Start')
        while True:
            try:
                rv = await self.col(True).find().to_list(None)
                await self.log.ainfo('End')
                return rv
            except self.mongo_exceptions as exception:
                await self.log.awarning(f'Waiting for connection: {repr(exception)}')
                await self.anap()
    ...
    async def find_field(self, field: str, filtering: typing.Any = None) -> dict:
        args = Frame.args()
        await self.log.ainfo('Start', args)
        while True:
            try:
                rv = dict()
                col = await self.col(True).find({field: True}, {'_id': 1, field: 1}).to_list(None)
                for item in col:
