...
class Env(Asdict):
    ...
    def __init__(self, prefix: str = str(), sem_factor: int = Sem._factor):
        ...
        for item in self.Var._fields:
            ...
            if item == 'log_level':
                ic(os.environ.get(name, field.default))
            value = function(*var)
            if item == 'log_level':
                ic(value)
        ...
        # self.ic = self.file.bool(upper_prefix('IC', prefix=prefix), self.debug_default)
        # self.log_level = self.file.log_level(upper_prefix('LOG_LEVEL', prefix=prefix), self.log_level_default)
        # self.log_level_file = self.file.log_level(upper_prefix('LOG_LEVEL_FILE', prefix=prefix),
        #                                           self.log_level_file_default)
...
class Kill:
    ...
    def count(self, recursive: bool =True, text: str = 'nmap'):
        try:
            for item in self.current_process.children(recursive=recursive):
                ic(item.cmdline())
            return len([True for item in self.current_process.children(recursive=recursive) if text in item.cmdline()])
        except (psutil.AccessDenied, psutil.NoSuchProcess) as exception:
            self.log.warning(f'{text=}', f'{exception=}')
