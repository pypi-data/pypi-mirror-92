...
def get_root_direct_child_nodes(xml_file, node_tag, node_att_name=None, node_att_val=None):
    return get_child_nodes(get_root_node(xml_file), node_tag, node_att_name, node_att_val)
...
def read_xml_file(xml_path, namespace_dict=None):
    xml = et.parse(xml_path)
    if namespace_dict:
        for prefix, uri in namespace_dict:
...
def find_all_nodes(parent_node, node_tag, node_att_name=None, node_att_val=None, recursive=True):
    selector = node_tag
    ...
    if recursive:
        ...
        return parent_node.findall(selector)
