define([
    ...
    $(document).on("click", vpCommon.wrapSelector(vpCommon.formatString("#{0}", vpConst.VP_NOTE_EXTRA_MENU_OPEN_NOTE)), function() {
        /** 2021 01 17 새로 추가*/
        // var saveFileName = $(vpCommon.wrapSelector(vpCommon.formatString("#{0}", vpConst.VP_NOTE_FILE_PATH_CONTROL))).attr("placeholder");
        // var saveFilePath = $(vpCommon.wrapSelector(vpCommon.formatString("#{0}", vpConst.VP_NOTE_REAL_FILE_PATH))).val();
        var saveFileName = $('#vp_apiblock_board_makenode_input').val();
        if ( saveFileName == '') {
            saveFileName = 'Untitled';
        }
        var saveFilePath = vpCommon.formatString("./{0}.{1}", saveFileName, vpConst.VP_NOTE_EXTENSION);
        apiBlockJS.openMultiBtnModal_new('Save', `Save changes to '${saveFileName}.vp'`,['Don`t Open', 'Open', 'Save And Open'], [() => {
    ...
    $(document).on("fileSaveSelected.fileNavigation", function(e) {
        ...
        if (e.path.substring(e.path.lastIndexOf(".") + 1) === vpConst.VP_NOTE_EXTENSION) {
            ...
            console.log('saveFileName', saveFileName);
            apiBlockJS.openMultiBtnModal_new('Save As', `Save changes to '${saveFileName}'`,['Don`t save','Cancel', 'Save'], [() => {
     ...
     * 노트 파일 저장
     */
    var saveNotePage_newversion = function() {
        var saveFileName = $('#vp_apiblock_board_makenode_input').val();
        if ( saveFileName == '') {
            saveFileName = 'Untitled';
        }
        ...
        apiBlockJS.openMultiBtnModal_new('Save', `Save changes to '${saveFileName}.vp'`,['Don`t save', 'Cancel', 'Save'], [() => {
