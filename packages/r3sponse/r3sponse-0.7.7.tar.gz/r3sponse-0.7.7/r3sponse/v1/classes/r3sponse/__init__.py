...
class R3sponse(object):
	...
	def check_parameter(self, parameter=None, name="parameter", empty_value=None):
		...
		if ":" in name:
			...
			param_format = Formats.get(parameter, serialize=False)
			if param_format not in formats:
				return self.error_response(f"Incorrect parameter [{name}] format, correct format(s): {Files.Array(path=False, array=formats).string(joiner=', ')}.")
...
class ResponseObject(object):
	...
	def __str__(self):
		...
		if isinstance(self, dict):
			...
			return json.dumps(self.dict(), indent=4)
	def json(self,):
		return json.dumps(self.dictionary(serializable=True), indent=4)
