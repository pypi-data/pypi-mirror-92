        ...
        for i, (b, t) in enumerate(z):
            if i == 0:
                ...
                if row and row not in rows:
                    rows.append(row)
        if sum([ len(r) for r in rows]) < sum([len(l) for l in self.textlines]):
            return self.extract_from_text()
        ...
        for group in left_right:
            ...
            while i < len(group):
                ...
                if line.space_bellow <= min_row_height:
                    ...
                    subgroup = []
                i += 1
                while i < len(group) and np.isclose(line.space_bellow, group[i].space_bellow, atol=line.space_above/2):
