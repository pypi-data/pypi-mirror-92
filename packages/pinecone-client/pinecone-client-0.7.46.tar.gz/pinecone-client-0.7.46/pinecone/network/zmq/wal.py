...
class WALServlet(ZMQServlet):
    ...
    def __init__(self, servlet_spec: ServletSpec, num_replicas: int, path: str):
    ...
    async def poll_sock(self, sock: zmq.Socket):
        while True:
            try:
                msg = await asyncio.wait_for(self.recv_msg(sock), RECV_TIMEOUT)
                if self.use_wal(msg):
    ...
    async def run_sync(self, func, *args):
        return await run_in_executor(asyncio.get_event_loop(), self.executor, functools.partial(func, *args))
