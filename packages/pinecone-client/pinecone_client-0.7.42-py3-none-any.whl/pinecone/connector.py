...
class Connection:
    ...
    def __init__(self, host: str, port: int, api_key: str = None, service_name: str = None, timeout: int = 0,
                 response_timeout: int = 2, traceroute: bool = False):
        """
        ...
        :param response_timeout: defaults to 2 seconds. Return failure if gateway doesn't receive response for
        this many seconds.
        :type response_timeout: int, optional
        :param traceroute: Whether to send receipts back to the gateway from each stage of the graph, and return the
        route if the gateway doesn't receive a response after response_timeout seconds.
