...
def stream_blocking_logs(arrow_container: Container, rest_container: Container) -> None:
    arrow_stream = blocking_generator(arrow_container.logs(stream=True), kind="arrow")
    rest_stream = blocking_generator(rest_container.logs(stream=True), kind="rest")
    try:
        for msg, color in merge_blocking_iterators((arrow_stream, rest_stream),):
...
def blocking_generator(
    container_logs: Generator[bytes, None, None], kind: str = "arrow"
    ...
    while True:
        try:
            ...
            return
        if kind == "arrow":
            yield f"> {line.decode()}".strip(), "bright_magenta"
        else:
            yield f"> {line.decode()}".strip(), "bright_green"
        if not line:
            return


def merge_blocking_iterators(
    generators: Tuple[Generator[tuple, None, None], ...]
