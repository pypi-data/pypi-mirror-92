...
class layer:
    ...
    def update_parameters_rmsprop(self, learning_rate, beta=0.9):
        self.s_w = beta*self.s_w +  (1 - beta)* np.power(grads['dW'+str(i+1)],2)
        self.s_b = beta*self.s_b +  (1 - beta)* np.power(grads['db'+str(i+1)],2)
        self.weights = self.weights - learning_rate * np.divide(grads['dW'+str(i+1)],np.sqrt(self.s_w))
        self.biases = self.biases -  learning_rate  *  np.divide(grads['db'+str(i+1)],np.sqrt(self.s_b))
