...
class Consumer:
    def __init__(self, db_uri, api_url, token):
    ...
    @staticmethod
    def callback(topic, name, data):
        def wrapper(event):
            event.add_tab("CONSUMER", {"name": name, "topic": topic, "data": data})
        return wrapper
    ...
    def _get_messages(self, topic, name):
        response = requests.get(f"{self.api_url}/message/{topic}?consumer={name}&token={self.token}")
    ...
    def _acknowledge_message(self, message_id, name):
        return requests.patch(f"{self.api_url}/message/{message_id}?consumer={name}&token={self.token}").json()
    ...
    def _consume(self, function, event, name):
        ...
        with self.db.watch([{'$match': {'operationType': 'insert'}}]) as stream:
            for insert_change in stream:
                if insert_change['fullDocument']['topic'] == topic:
                    data = self._get_messages(topic, name)
                    while data:
                        bugsnag.before_notify(self.callback(topic, name, data))
                        new_data = event(**data['payload'])
