...
RB.UploadAttachmentView = RB.DialogView.extend({
    ...
    template: _.template(['<div class="formdlg" style="width: 50em;">', ' <div class="error" style="display: none;"></div>', ' <form encoding="multipart/form-data" enctype="multipart/form-data"', '       id="attachment-upload-form">', '  <table>', '   <tbody>', '    <tr>', '     <td class="label"><label><%- captionText %></label></td>', '     <td><input name="caption" type="text" value="<%- presetCaption %>"></td>', '     <td><ul class="errorlist" style="display: none;"></ul></td>', '    </tr>', '    <tr>', '     <td class="label">', '      <label class="required"><%- pathText %></label>', '     </td>', '     <td><input name="path" type="file" class="js-path"></td>', '     <td><ul class="errorlist" style="display: none;"></ul></td>', '    </tr>', '   </tbody>', '  </table>', '  <% if (attachmentHistoryID >= 0) { %>', '    <input type="hidden" name="attachment_history"', '           value="<%- attachmentHistoryID %>" />', '  <% } %>', ' </form>', '</div>'].join('')),
        ...
        if (rsp && rsp.fields) {
            ...
            for (fieldName in rsp.fields) {
                if (rsp.fields.hasOwnProperty(fieldName)) {
                    $errorList = this.$(".errorlist").css("display", "block")[nameToRow[fieldName]];

                    for (i = 0; i < rsp.fields[fieldName].length; i++) {
                        $("<li/>").html(rsp.fields[fieldName][i]).appendTo($errorList);
                    }
                }
            }
        }
