...
class BaseDao(object):
            ...
            if kwargs:
                ...
                for k, v in kwargs.items():
                    if type(v) is list:
                        ...
                        filters.append(getattr(self.entry, k) == v)
                res = session.query(self.entry).filter(*filters).all()
            elif or_list:
                arg_list = []
                for one in or_list:
                    arg_list.append(eval("{}.{}=={}".format(self.entry.__name__, one[0], one[-1])))
                res = session.query(self.entry).filter(or_(*arg_list)).all()
            else:
                res = session.query(self.entry).all()
            self.tr.end()
        except:
            traceback.print_exc()
            session.rollback()
    ...
    def select_execute(self, sql, num=0, *args, **kwargs):
        session = self.tr.start()
...
class ObjectCovertToDictDao(BaseDao):
    ...
    def simple_get_all_filter(self, params, condition_type='and',  **kwargs):
        ...
        if type(res) == tuple:
            return [one.to_dict() for one in res[0]],res[1]
        return res
