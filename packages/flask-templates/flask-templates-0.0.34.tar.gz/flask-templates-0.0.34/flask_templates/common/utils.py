...
def mix_change(src_iterator, desc_iterator, total=dict(), pre=""):
    """
    复合替换dict
    :param src_iterator:
    :param desc_iterator:
    ...
    if type(src_iterator) == dict:
        ...
        for key in all_dict.keys():
            ...
            if type(value) == dict:
                mix_change(value, desc_iterator.get(key, {}), total=total, pre=pre + key + ".")
            elif type(value) == list:
                mix_change(value, desc_iterator.get(key, None), total=total, pre=pre + key + ".")
            else:
                value = desc_iterator.get(key, value)
                value = replace_variables(value, **total)
                total[pre + key] = value
                src_iterator[key] = value
                # src_iterator[key] = desc_iterator.get(key, value)
    elif type(src_iterator) == list:
        # 列表暂时也用替换
        if desc_iterator is not None:
            src_iterator.clear()
            # todo 暂时不支持替换list内的变量
            src_iterator.extend(desc_iterator)
        replace_variables_collection(src_iterator,**total)
    else:
...
def replace_variables(src, **variables_dict):
    """
    ...
    for replace_item, piece in match_obj:
        pieces = piece.split(":")
        value = variables_dict.get(pieces[0])
        if not value and len(pieces) > 1:
            value = pieces[1]
    ...
    if type(src_iterator) == dict:
        for key, value in src_iterator.items():
            ...
            if type(value) == dict or type(value) == list:
                iter_change(value, name_dict, name_list)
            else:
        ...
        for one in src_iterator:
            ...
            if type(one) == dict or type(one) == list:
                iter_change(one, name_dict, name_list)
            else:
                env_key = sep.join(name_list)
