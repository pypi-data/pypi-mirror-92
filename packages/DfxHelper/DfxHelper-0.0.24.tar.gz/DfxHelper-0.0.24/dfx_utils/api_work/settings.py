from dfx_utils.helper import BaseData, SuperDict, singleton
...
class ApiSetting(SuperDict):
        ...
        for key, val in setting.items():
            if isinstance(val, dict):
                ...
                if not getattr(data, key) and val[-1]:
                    ...
                    raise Exception(f'data({key}) need {val[0]}, found {type(getattr(data, key))}')
    ...
    def __check(self, typ, data: BaseData):
        ...
        for item in self.get(typ, []):
            ...
            if value is None:
                raise Exception(f'{typ}({item}) not found')
...
class ApiSettingManager(dict):
    ...
    def __call__(self, api_name, path, method) -> ApiSetting:
        ...
        self[api_name] = ApiSetting(api_name, request_url, method)
