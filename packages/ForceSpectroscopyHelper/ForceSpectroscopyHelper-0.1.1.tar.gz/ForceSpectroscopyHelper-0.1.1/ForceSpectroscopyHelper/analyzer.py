...
class BaseAnalyzer:
    ...
    """
    @staticmethod
    def fitting_map(mapping_data, flatten=FlattenMode.Average, smooth=False, flatten_param=None):
    ...
    def fitting_data(pixel_data: List[Vector3], size: Vector2, mapping_data=None,
                     flatten=FlattenMode.Average, smooth=False, flatten_param=None) -> List[Vector3]:
        ...
        if flatten == BaseAnalyzer.FlattenMode.LinearFit:
            ...
            root.deiconify()
            root.mainloop()
            print(float(X_Field.get()), float(Y_Field.get()))
            mapping_data = BaseAnalyzer.flatten_plane(mapping_data, float(X_Field.get()), float(Y_Field.get()))
