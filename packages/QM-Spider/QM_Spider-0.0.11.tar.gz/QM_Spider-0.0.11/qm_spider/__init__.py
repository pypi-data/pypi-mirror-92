...
class get_keyword_info:
    def __init__(self, keyword, run_time=datetime.date.today(), version='ios12', country='cn', device='iphone', search_type='all'):
    ...
    def get_keyword_search(self):
        url = 'https://api.qimai.cn/search/index?device=%s&country=%s&search=%s&version=%s&date=%s&search_type=%s&edate=%s' %(self.device, self.country, self.keyword, self.version, self.run_time, self.search_type, self.run_time-self.one_day)
        res = session.get(url, headers=headers)
        self.keyword_serch_index = res.json()
        return self.keyword_serch_index

    def get_keyword_WordInfo(self):
        url = 'https://api.qimai.cn/search/getWordInfo?device=%s&country=%s&search=%s&version=%s&date=%s&search_type=%s&edate=%s' %(self.device, self.country, self.keyword, self.version, self.run_time, self.search_type, self.run_time-self.one_day)
        res = session.get(url, headers=headers)
