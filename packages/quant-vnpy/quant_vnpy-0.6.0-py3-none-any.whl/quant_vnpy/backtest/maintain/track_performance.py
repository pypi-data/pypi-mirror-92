...
from quant_vnpy.backtest.commons import bulk_backtest_with_backtest_params_iter
from quant_vnpy.db.orm import StrategyBacktestStats, StrategyBacktestStatsArchive
...
def daily_backtest_all_strategies():
    ...
    for (module_name, strategy_class_name, symbols), stats_list in stats_list_dic.items():
        ...
        if multi_symbols:
            from quant_vnpy.backtest.portfolio_strategy.run import default_engine_param_key_func
        else:
            from quant_vnpy.backtest.cta_strategy.run import default_engine_param_key_func
