...
from quant_vnpy.backtest import DEFAULT_STATIC_ITEMS, CleanupOrcaServerProcessIntermittent
from quant_vnpy.backtest.cta_strategy.engine import BacktestingEngine as CtaBacktestingEngine
from quant_vnpy.backtest.portfolio_strategy.engine import BacktestingEngine as PortfolioBacktestingEngine
from quant_vnpy.config import logging
from quant_vnpy.constants import SYMBOL_SIZE_DIC, INSTRUMENT_PRICE_TICK_DIC, INSTRUMENT_RATE_DIC
from quant_vnpy.db.orm import set_account_backtest, StrategyBacktestStats, StrategyBacktestStatusEnum
...
def run_backtest(
    ...
    image_file_name = get_output_file_path(
        vt_symbol_str, cross_limit_method.name, f'{file_name_header}.png', root_folder_name=root_folder_name)
    stat_file_name = get_output_file_path(
        vt_symbol_str, 'stat', f'{file_name_header}_stat.md', root_folder_name=root_folder_name)
    daily_result_file_name = get_output_file_path(
        vt_symbol_str, 'data', f'{file_name_header}_result.csv', root_folder_name=root_folder_name)
    param_file_name = get_output_file_path(
        vt_symbol_str, 'param', f'{file_name_header}_param.json', root_folder_name=root_folder_name)
