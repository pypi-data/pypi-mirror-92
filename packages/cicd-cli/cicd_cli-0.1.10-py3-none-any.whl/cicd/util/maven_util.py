...
def deps():
    if not which('mvn'):
        if sys.platform == 'darwin':
            ...
            path = download_util.download_to_dir(download_url, config.M2_HOME)
            file_util.untar_file(path)
            file_util.add_to_bin(f"{config.M2_HOME}/apache-maven-3.6.3/bin/mvn", symlink=True)
            file_util.rm(f"{config.M2_HOME}/apache-maven-3.6.3-bin.tar.gz")
