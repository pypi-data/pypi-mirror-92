...
class connect_flow_FlowStore:
    ...
    __slots__ = ("delegate", "storeRequestOnFailure")
    _hx_fields = ["delegate", "storeRequestOnFailure"]
    _hx_methods = ["setStoreRequestOnFailure", "storesRequestOnFailure", "requestDidBegin", "requestDidSkip", "removeStepData", "getStepParam"]
    ...
    def requestDidSkip(self,request,data,index,attempt):
        if self.storesRequestOnFailure():
            result = connect_storage_StepStorage.save(request,connect_storage_StepData(index,data,connect_storage_StorageType.ConnectStorage,attempt),self.getStepParam(request),Reflect.field(request,"update"))
            if (self.delegate is not None):
                tmp = result.index
                if (tmp == 0):
