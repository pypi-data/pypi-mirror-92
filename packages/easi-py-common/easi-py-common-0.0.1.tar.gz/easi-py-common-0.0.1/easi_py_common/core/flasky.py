...
from core.env import EnvHelper
from werkzeug.utils import find_modules, import_string

from core.error import ServiceException, intercept_service_exception
from core.redis import redis_store, redis_ro_store
from core.sql_alchemy import db
from dantic.monkey_doc import validate
...
class AppWrapper:
    ...
    def _create_app(self, name: str) -> Flask:
        ...
        # from service.push.services.leancloud import courier_leancloud_service
        # courier_leancloud_service.init_app(app)

        app_path = os.path.dirname(app.root_path)
        en_yml = os.path.join(app_path, 'common/i18n/en.yml')
        cn_yml = os.path.join(app_path, 'common/i18n/cn.yml')
        ja_yml = os.path.join(app_path, 'common/i18n/ja.yml')
        ms_yml = os.path.join(app_path, 'common/i18n/ms.yml')
        with open(en_yml) as i18n_en_file:
            i18n_en = yaml.load(i18n_en_file, Loader=yaml.FullLoader)

        with open(cn_yml) as i18n_cn_file:
            i18n_cn = yaml.load(i18n_cn_file, Loader=yaml.FullLoader)

        with open(ja_yml) as i18n_ja_file:
            i18n_ja = yaml.load(i18n_ja_file, Loader=yaml.FullLoader)

        with open(ms_yml) as i18n_ms_file:
            i18n_ms = yaml.load(i18n_ms_file, Loader=yaml.FullLoader)

        app.i18n = {'en': i18n_en, 'cn': i18n_cn, 'ja': i18n_ja, 'ms': i18n_ms}
