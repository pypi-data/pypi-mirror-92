...
def service_mapping(uri, method, dest_type=None):
    def wrapper(f):
        try:
        ...
        def wrapped(*args, **kwargs):
            ...
            if method == HTTP_METHOD_GET:
                return client.get(_uri, params=data, dest_type=dest_type)
            elif method == HTTP_METHOD_POST:
                return client.post(_uri, json_data=data, dest_type=dest_type)
            elif method == HTTP_METHOD_PUT:
                return client.put(_uri, json_data=data, dest_type=dest_type)
            elif method == HTTP_METHOD_DELETE:
                return client.delete(_uri, dest_type=dest_type)
            else:
