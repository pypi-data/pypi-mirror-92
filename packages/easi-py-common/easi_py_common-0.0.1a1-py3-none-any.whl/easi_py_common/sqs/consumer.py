...
from pubsub import queue_constants
from pubsub.core.producer import producer
from pubsub.queue_constants import ConsumerLogConstants
...
class ConsumerStart:
    ...
    def __save_sub_log(self, sub_log_ret):
        try:
            sub_log_len = redis_store.lpush(ConsumerLogConstants.SUB_LOG, json.dumps(sub_log_ret))
            ...
            if can_bulk_save or sub_log_len % 200 == 0 or sub_log_len >= ConsumerLogConstants.CONSUMER_LOG_MAX_SLEEP_ES_BULK_BUFFER_SIZE:
                producer.send(queue_constants.SUB_LOG_BULK_SAVE, '')
        except Exception:
            pass
