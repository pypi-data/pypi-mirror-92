...
class AbberationsFit(DataContainer):
    ...
    * wavelength : Incoming beam's wavelength [m].
    ...
    :func:`scipy.optimize.least_squares` : Full nonlinear least-squares
        algorithm description.
    """
    attr_set = {'defocus', 'distance', 'wavelength', 'phase',
                'pixel_abberations', 'pixel_size', 'roi'}
    ...
    def phase_to_pix(self, fit):
        ...
        numpy.ndarray
            Lens` abberations phase fit coefficients.
        """
        nfit = self.wavelength * fit[:-1] / (2 * np.pi * self.pix_ap**2 * self.defocus)
        nfit *= np.arange(1, fit.size)[::-1]
        return nfit
    ...
    def fit(self, max_order=2, xtol=1e-14, ftol=1e-14, loss='cauchy', roi=None):
        ...
        if ph_fit.size >= 4:
            ...
            alpha = None
        return {'alpha': alpha, 'fit': fit, 'ph_fit': ph_fit, 'rel_err': err / fit, 'r_sq': r_sq}
    ...
    def fit_phase(self, max_order=3, xtol=1e-14, ftol=1e-14, loss='cauchy', roi=None):
        ...
        if ph_fit.size >= 4:
            ...
            alpha = None
        return {'alpha': alpha, 'fit': fit, 'ph_fit': ph_fit, 'rel_err': err / ph_fit, 'r_sq': r_sq}
