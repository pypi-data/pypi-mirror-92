...
from ..bin import barcode_steps, barcode_profile
...
class STParams(INIParser):
    ...
    **kwargs : dict
    ...
    def y_wavefront_size(self):
        ny_ltos = int(4 * self.ap_y**2 * max(self.focus, self.defocus) / self.focus**2 / self.wl)
    ...
    def barcode_steps(self, dist):
        x0, x1 = self.beam_span(dist)
        return barcode_steps(x0=x0 + self.offset, b_dx=self.bar_size, rd=self.rnd_dev,
