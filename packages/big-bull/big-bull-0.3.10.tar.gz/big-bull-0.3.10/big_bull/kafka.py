...
def get_kafka_consumer_span(message, consumer, tracer):
    ...
    span = tracer.start_span(
        operation_name=f"from_{message.topic}", reference=opentracing.follows_from(span_context)
    )
...
async def kafka_consumer_task(func, injectables, *args, **kwargs):
    ...
    try:
        ...
        async for message in consumer:
            await graph.inject_func(func, injectables, message=message, consumer=consumer)
    finally:
        await consumer.stop()
