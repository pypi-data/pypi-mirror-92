...
class Asset(KitsuAsset):
    ...
    def get_default_contextual_edits(self, context_name):
        if context_name == "settings":
            default_settings = get_contextual_dict(self._assets, context_name)
            path = default_settings.get("path", "UNKNOWN_PATH")

            return dict(path=os.path.join(path, self.name()), asset_name=self.name())
...
class AssetFamily(flow.Object):
    ...
    def get_default_contextual_edits(self, context_name):
        if context_name == "settings":
            default_settings = get_contextual_dict(self._asset_type, context_name)
            path = default_settings.get("path", "UNKNOWN_PATH")

            return dict(
                path=os.path.join(path, self.name()), asset_family=self.name().lower()
            )
...
class AssetType(flow.Object):
    ...
    def get_default_contextual_edits(self, context_name):
        if context_name == "settings":
            default_settings = get_contextual_dict(self._lib, context_name)
            path = default_settings.get("path", "UNKNOWN_PATH")

            return dict(
                path=os.path.join(path, self.name()), asset_type=self.name().lower()
            )
...
class AssetLib(flow.Object):
    ...
    def get_default_contextual_edits(self, context_name):
        if context_name == "settings":
            return dict(path="LIB", CATEGORY="LIB")
