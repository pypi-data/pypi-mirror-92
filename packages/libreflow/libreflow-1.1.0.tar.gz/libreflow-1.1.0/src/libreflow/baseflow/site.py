...
from libreflow.baseflow.runners import BaseRunner
...
class Request(flow.Action):
    ...
    def run(self, button):
        ...
        if current_site_status.status.get() != "NotAvailable":
            self.message.set("Revision status: %s" % current_site_status.status.get())
        ...
        for site_status in self._revision.sync.mapped_items():
            if site_status.status.get() == "Available":
                self.root().session().log_debug("Found revision in site %s" % site_status.name())
                requested_site = sites[site_status.name()]

        if not requested_site:
            self.message.set("Revision unavailable yet")
            return self.get_result(close=False)
        ...
        for site_name in self._revision.sync.mapped_names():
            ...
            if site.site_type.get() == "Exchange" and site_status.status.get() == "Available":
                self.message.set("Revision already available on exchange server")
                return self.get_result(close=False)
        ...
        for job in requested_site.queue.mapped_items():
            if job.emitter_oid.get() == self._revision.oid() and job.type.get() == "Upload":
                self.message.set("Revision already requested for upload in source site")
                return self.get_result(close=False)
