...
from .data import get_layer, get_covid19, get_data, get_daily_mobility_matrix, get_population, get_zone_movements
...
def download_layer(layer, output_file, plot=False, no_save=False):
    ...
    featureCollection = get_layer(layer, print_url=True)
...
def download_covid19(ev, output_file, output_format='csv', start_date=None, end_date=None):
    ...
    data = get_covid19(ev, start_date=start_date, end_date=end_date, fmt='records', print_url=True)
...
def download_data(ev, output_file, output_format='csv', start_date=None, end_date=None):
    ...
    data = get_data(ev, start_date=start_date, end_date=end_date, fmt='records', print_url=True)
    if output_format == 'csv':
        df = pd.DataFrame(data)
        df.to_csv(output_file, index=False)
        print(f'{df.shape[0]} rows written to file:', output_file)
...
def download_daily_mobility_matrix(source_layer, target_layer, date, output_file, output_format='csv', source=None, target=None):
    ...
    data = get_daily_mobility_matrix(source_layer, target_layer, date, fmt='records', print_url=True, source=source, target=target)
    if output_format == 'csv':
        df = pd.DataFrame(data)
        df.to_csv(output_file, index=False)
        print(f'{df.shape[0]} rows written to file:', output_file)
...
def download_population(layer, output_file, output_format='csv', start_date=None, end_date=None):
    ...
    data = get_population(layer, start_date=start_date, end_date=end_date, fmt='records', print_url=True)
...
def download_zone_movements(layer, output_file, output_format='csv', start_date=None, end_date=None):
    print(f'Dowloading population for layer={layer}')
    data = get_zone_movements(layer, start_date=start_date, end_date=end_date, fmt='records', print_url=True)
