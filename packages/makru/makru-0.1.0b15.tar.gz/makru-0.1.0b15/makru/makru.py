...
class Makru(object):
    ...
    def search_config_path(cls, arguments: List[str]):
        ...
        for v in list(map(lambda x: arguments[x], config_indexs)): # .remove also cause indexes changes, we need a "snapshot" of values
            arguments.remove(v) # Every .pop() cause the change of indexes, so here use value to remove the element
        return result
