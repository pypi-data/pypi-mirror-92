...
def train(args: argparse.Namespace) -> Optional[Text]:
    ...
    domain = convo.cli.utils.get_validated_path(
    ...
    training_files = [
        convo.cli.utils.get_validated_path(
...
def train_core(
    ...
    from convo.train import train_core

    output = train_path or args.out

    args.domain = convo.cli.utils.get_validated_path(
        args.domain, "domain", DEFAULT_DOMAIN_PATH, none_is_valid=True
    )
    story_file = convo.cli.utils.get_validated_path(
...
def train_nlu(
    ...
    nlu_data = convo.cli.utils.get_validated_path(
...
def _get_valid_config(
    ...
    """
    config = convo.cli.utils.get_validated_path(config, "config", default_config)
    ...
    missing_keys = convo.cli.utils.missing_config_keys(config, mandatory_keys)
    if missing_keys:
        print_error(
