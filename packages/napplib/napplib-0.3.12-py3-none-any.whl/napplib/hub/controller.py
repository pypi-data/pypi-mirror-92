...
class HubController:
    ...
    @classmethod
    def get_all_products(self, server_url='', token='', limit=''):
        # create headers
        headers = dict()
        headers['Authorization'] = f'Bearer {token}'

        if limit != '':
            r = requests.get(f'{server_url}/products/{store_id}/?offset=0&limit={limit}', headers=headers, timeout=180)
            r = r.content.decode('utf-8')
    ...
    def check_store_products_marketplace_status(self, server_url='', token='', status='', marketplaceId=0, storeId=0):
        ...
        if status == 'pending_register_product':
            ...
            response_update = r['data']

        return response_new, response_update

    @classmethod
    def get_all_store_products(self, server_url='', token='', store_id='', limit=''):
        # create headers
        headers = dict()
        headers['Authorization'] = f'Bearer {token}'

        if limit != '':
            r = requests.get(f'{server_url}/storeProductsByStore/{store_id}/?offset=0&limit={limit}', headers=headers, timeout=180)
            r = r.content.decode('utf-8')
    ...
    def update_store_product(self, server_url, token, inventory, store_product_id, product_id):
        ...
        # log
        logging.info(f'Atualizando o inventário da loja... [{"sucesso ao atualizar o inventário (200)" if r.status_code == 200 else r.status_code}] - {inventory.id}')
    ...
    def check_inventory(self, server_url, token, store_products=[], store_id=0, marketplace_id=0, mkt_category='', use_ean=False, use_sku=False, use_mkt=False):
        ...
        for store_product in store_products:
            invetaryEan = store_product.productEan['String']
