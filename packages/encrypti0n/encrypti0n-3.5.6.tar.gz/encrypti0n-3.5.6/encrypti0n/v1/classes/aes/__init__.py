...
class AsymmetricAES(object):
	...
	def decrypt(self, string, decode=False):
		...
		#
	def encrypt_file(self, input=None, output=None, remove=False):
		...
		# encrypt.
		response = self.encrypt(utils.__load_file__(input), decode=False)
		if not r3sponse.success(response): return response
		...
		#
	def decrypt_file(self, input=None, output=None, remove=False):
		...
		# encrypt.
		response = self.decrypt(utils.__load_file__(input), decode=False)
		if not r3sponse.success(response): return response

		# write out.
		try: utils.__save_bytes__(output, response["decrypted"])
