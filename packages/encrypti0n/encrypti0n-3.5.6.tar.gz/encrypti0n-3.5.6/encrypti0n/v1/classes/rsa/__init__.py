...
class RSA(object):
	def __init__(self, 
		...
		if directory != None:
			...
			directory = Formats.FilePath(private_key).base()
	...
	def generate_keys(self):
		...
		# response.
		return r3sponse.success_response(f"Successfully generated a key {directory}.")
	...
	def edit_passphrase(self, passphrase=None):
		...
		if passphrase == None:
			passphrase = '""'
		output = utils.__execute__(["ssh-keygen", "-p", "-P", f"{self.passphrase}", "-N", f"{passphrase}", "-f", f"{self.private_key}"])
