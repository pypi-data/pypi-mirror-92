...
class TrackHandler(RequestHandler):
    ...
    async def validate(self, track_info, api_key):
        ...
        if last_tracted:
            delta = datetime.utcnow() - last_tracted
            # rate_limit = 120
    ...
    async def track(self, track_info, api_key):
        ...
        try:
            ...
            print("Track failed for user with API key {}: ".format(api_key, e))
            return
        except AssertionError as e:
            print("Track failed for user with API key {} due to due to client error: {}".format(api_key, e))
            self.write_error(400, message=e)
            return
        ...
        results = await self.db.query("INSERT INTO track_questions (track_id, assignment_id, class_id, question_id, score, user_id, timestamp) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                                            [track_id, assignment_id, class_id, question_id, score, user_id, datetime.utcnow()])
