...
class WebsocketSession(object):
    ...
    def _check_auto_ping(self, ping_rate, ping_data, session_time):
        ...
        if ping_rate and session_time > self._next_ping:
            ...
            try:
                self.websocket.send_ping(data=ping_data)
    ...
    def _regular(self, poll, ping_rate, ping_data, ping_timeout, close_timeout):
        """Run regularly to do polling / pings."""
        # Check for regularly running actions.
        if self._check_poll(poll, self.session_time):
            yield events.Poll()
        self._check_auto_ping(ping_rate, ping_data, self.session_time)
        ...
        def _regular():
            ...
            if self._ready:
                return self._regular(
                    poll, ping_rate, ping_data, ping_timeout, close_timeout
                )
            return ()
