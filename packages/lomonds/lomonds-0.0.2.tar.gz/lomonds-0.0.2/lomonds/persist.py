...
def persist(websocket, poll=5,
            ...
            ping_rate=30, ping_data= None, ping_timeout=None,
            exit_event=None):
    ...
    :param float ping_rate: Delay between pings (seconds), or `0` for no
        auto ping.
    :param byte ping_data: data used in ping package.
    ...
    while True:
        ...
        for event in websocket.connect(
                poll=poll, ping_rate=ping_rate, ping_data=ping_data, ping_timeout=ping_timeout):
            if event.name == 'ready':
