...
class Model(ctypes.Structure):
	...
	def export_onnx(self,fname,probmodel=False):
		...
		for e in range(self.graph.edges):
			...
			WEIGHTS = helper.make_tensor_value_info(WEIGHTSid, TensorProto.DOUBLE, shape=(Ys,Yt))	
			...
			EXPWEIGHTS = helper.make_tensor_value_info(EXPWEIGHTSid, TensorProto.DOUBLE, shape=(Ys,Yt))
			...
			tEXPWEIGHTS = helper.make_tensor_value_info(tEXPWEIGHTSid, TensorProto.DOUBLE, shape=(Yt,Ys))
		...
		CONSTANT_ONE  = helper.make_tensor_value_info(CONSTANT_ONEid, TensorProto.INT64, shape=(1,))
		CONSTANT_ONEv = numpy.array([1],dtype=numpy.int64)
		...
		ONECOLD_VALUES = helper.make_tensor_value_info(ONECOLD_VALUESid, TensorProto.INT64, shape=(2,))
		ONECOLD_VALUESvals = numpy.array([1,0],dtype=numpy.int64)
		...
		ONEHOT_VALUES = helper.make_tensor_value_info(ONEHOT_VALUESid, TensorProto.INT64, shape=(2,))
		ONEHOT_VALUESvals = numpy.array([0,1],dtype=numpy.int64)
		...
		for v in range(self.graph.nodes):
			...
			NNid = 'NN_'+str(v)
			NN = helper.make_tensor_value_info(NNid, TensorProto.INT64, shape=(1,1))
			NNval = numpy.array([nn],dtype=numpy.int64)
			...
			NUMid = 'NUM_'+str(v)
			NUM = helper.make_tensor_value_info(NUMid, TensorProto.INT64, shape=(1,1))
			NUMval = numpy.array([v],dtype=numpy.int64)
			...
			for iw in range(nn):
				...
				IDX = helper.make_tensor_value_info(IDXid, TensorProto.INT64, shape=(1,))
				IDXval = numpy.array([iw],dtype=numpy.int64)
				...
				FMASKED_NEIGHBORS = helper.make_tensor_value_info(FMASKED_NEIGHBORSid, TensorProto.INT64, shape=(nn,1))
				...
				MASKED_NEIGHBORS = helper.make_tensor_value_info(FMASKED_NEIGHBORSid, TensorProto.DOUBLE, shape=(nn,1))
		...
		NUM = helper.make_tensor_value_info(NUMid, TensorProto.INT64, shape=(1,1))
		NUMval = numpy.array([self.graph.nodes],dtype=numpy.int64)
		NUMn = helper.make_node(
			'Constant',
			inputs=[],
			outputs=[NUMid],
		...
		OBSERVED = helper.make_tensor_value_info('observed_data', TensorProto.INT64, shape=(self.graph.nodes,))
...
#		DBG_SLICES = []
		...
		for v in range(self.graph.nodes):
			...
			for w in Ne[v]:
				...
				INITIALMSG = helper.make_tensor_value_info(INITIALMSGid, TensorProto.DOUBLE, shape=(Yv,1))
				INITIALMSGval = numpy.zeros(Yv).reshape(Yv,1)
			...
			XVid = 'XV'+str(v)
			XV  = helper.make_tensor_value_info(XVid, TensorProto.INT64, shape=(1,))
			XVv = numpy.array([Yv],dtype=numpy.int64)
			...
			VERTEX_SLICE = helper.make_tensor_value_info(VERTEX_SLICEid, TensorProto.INT64, shape=(Yv,1))
			...
			FIS_NOT_OBSERVED = helper.make_tensor_value_info(FIS_NOT_OBSERVEDid, TensorProto.INT64, shape=(1,1))
			...
			IS_NOT_OBSERVED = helper.make_tensor_value_info(IS_NOT_OBSERVEDid, TensorProto.DOUBLE, shape=(1,1))
			...
			FOBSVEC = helper.make_tensor_value_info(FOBSVECid, TensorProto.INT64, shape=(Yv,1))
			...
			tFOBSVEC = helper.make_tensor_value_info(tFOBSVECid, TensorProto.INT64, shape=(1,Yv))
			...
			OBSVEC = helper.make_tensor_value_info(OBSVECid, TensorProto.DOUBLE, shape=(1,Yv))
		...
		for I in range(ITERS):
			for v in range(self.graph.nodes):
				...
				INCOMINGS = helper.make_tensor_value_info(INCOMINGSid, TensorProto.DOUBLE, shape=(Yv,nn))
				...
				for iw in range(nn):
					...
					MSGSUM = helper.make_tensor_value_info(MSGSUMid, TensorProto.DOUBLE, shape=(Yv,1))
					...
					EXPMSGSUM = helper.make_tensor_value_info(EXPMSGSUMid, TensorProto.DOUBLE, shape=(Yv,1))
					...
					tEXPMSGSUM = helper.make_tensor_value_info(tEXPMSGSUMid, TensorProto.DOUBLE, shape=(1,Yv))
					...
					StEXPMSGSUM = helper.make_tensor_value_info(StEXPMSGSUMid, TensorProto.DOUBLE, shape=(1,Yv))
					...
					INOBSVEC = helper.make_tensor_value_info(INOBSVECid, TensorProto.DOUBLE, shape=(1,Yv))
					...
					MULMSG = helper.make_tensor_value_info(MULMSGid, TensorProto.DOUBLE, shape=(Yw,Yv))
					...
					if probmodel:
						...
						OUTMSG = helper.make_tensor_value_info(OUTMSGid, TensorProto.DOUBLE, shape=(Yw,1))
						...
						# The current runtime does not support ReduceMax with TensorProto.DOUBLE
						
						OUTMSG1id = 'MSG1_'+str(v)+'_'+str(w)+'_'+str(I)
						OUTMSG1 = helper.make_tensor_value_info(OUTMSG1id, TensorProto.DOUBLE, shape=(Yw,Yv))
						...
						OUTMSG1b = helper.make_tensor_value_info(OUTMSG1bid, TensorProto.DOUBLE, shape=(Yw,1))
					...
					ZOUTMSG = helper.make_tensor_value_info(ZOUTMSGid, TensorProto.DOUBLE, shape=(1,))
					...
					NORMOUTMSG = helper.make_tensor_value_info(NORMOUTMSGid, TensorProto.DOUBLE, shape=(Yw,1))
					...
					FINALOUTMSG = helper.make_tensor_value_info(FINALOUTMSGid, TensorProto.DOUBLE, shape=(Yw,1))
		...
		for v in range(self.graph.nodes):
			...
			INSUM = helper.make_tensor_value_info(INSUMid, TensorProto.DOUBLE, shape=(Yv,))
			...
			ExpINSUM = helper.make_tensor_value_info(ExpINSUMid, TensorProto.DOUBLE, shape=(Yv,))
			...
			SINSUM = helper.make_tensor_value_info(SINSUMid, TensorProto.DOUBLE, shape=(Yv,))
			...
			MSINSUM = helper.make_tensor_value_info(INOBSVECid, TensorProto.DOUBLE, shape=(Yv,))
			...
			ZExpINSUM = helper.make_tensor_value_info(ZExpINSUMid, TensorProto.DOUBLE, shape=(1,))
			...
			MARGINALS = helper.make_tensor_value_info(MARGINALSid, TensorProto.DOUBLE, shape=(Yv,))
			...
			rsMARGINALS = helper.make_tensor_value_info(rsMARGINALSid, TensorProto.DOUBLE, shape=(Yv,))
...
#			DBG_SLICES.append(MARGINALSid)
			...
			AMAXMARGINALS = helper.make_tensor_value_info(AMAXMARGINALSid, TensorProto.INT64, shape=(1,))
		...
		if probmodel:
			PROBid = 'PROB'
			s = numpy.sum(self.states,dtype=numpy.int64)
			OUTPUT = helper.make_tensor_value_info(PROBid, TensorProto.DOUBLE, shape=(int(s),))
			...
			MAPid = 'MAP'
			OUTPUT = helper.make_tensor_value_info(MAPid, TensorProto.INT64, shape=(self.graph.nodes,))
...
#		DBGid = 'DBG'
#		DBG = helper.make_tensor_value_info(DBGid, TensorProto.DOUBLE, shape=(8,3))
...
#		ONNX_OP_LIST.append(DBGn)
		...
		onnxmodel = helper.make_model(graph_def, producer_name='pxpy')
