...
def bins(series, points, min_bins=10):
    ...
    # adjust bin_size
    range_factor = np.ceil(min_bins/((series.max()-series.min())/bin_size))
    #range_factor = round(sum(([x/((series.max()-series.min())/bin_size) for x in bin_range]))/2)
