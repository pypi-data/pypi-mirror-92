...
import bluebelt.helpers.histogram as histogram
...
class ProcessCapability():
    ...
    def __repr__(self):
        ...
        return (f'{self.__class__.__name__}(n={self.sample_size}, target={self.target}, {lsl_text}={self.lsl}, {usl_text}={self.usl}, observed={self.observed_performance})')
    ...
    def calculate(self):
        ...
        self.mean = mean
        self.std = std
        self.lt_lsl = (self.series[self.series < self.lsl].count() / self.sample_size)
        self.gt_usl = (self.series[self.series > self.usl].count() / self.sample_size)
        self.observed_performance = self.series[(self.series >= self.lsl) & (self.series <= self.usl)].count() / self.sample_size
        self.pp = (self.usl - self.lsl) / (6 * self.std) # Pp = (USL – LSL) / 6 * sigma
        self.ppk = (min(self.mean - self.lsl, self.usl - self.mean)) / (3 * self.std) # Ppk = [ min(USL – x(bar), x(bar) - LSL) ] / 3 s
    ...
    def plot(self, **kwargs):
        ...
        if min_bins:
            ...
            bins = histogram.bins(self.series, histogram_points, min_bins)
        else:
            bins = None
        ...
        for i, patch in enumerate(patches):
            if bins[i] <= self.lsl or bins[i] >= self.usl:
                patch.set_color(style.process_capability.histogram_patch_edge_color)
                patch.set_hatch(style.process_capability.histogram_patch_edge_hatch)
