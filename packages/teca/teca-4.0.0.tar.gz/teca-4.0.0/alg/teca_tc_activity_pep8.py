...
class teca_tc_activity(teca_python_algorithm):
    ...
    def get_execute_callback(self):
        ...
        """
        def execute(port, data_in, req, streaming):
            """
            ...
            self.accum_by_year_and_region(
                year, ureg, year, region_id, start_y, ACE, regional_ACE)

            self.accum_by_year_and_region(
                uyear, ureg, year, region_id, start_y, PDI, regional_PDI)
            ...
            self.plot_individual(
                year, ureg, regional_ACE, 'ACE', '$10^4 kn^2$')

            self.plot_individual(
                year, ureg, regional_PDI, 'PDI', '$m^3 s^{-2}$')

            self.plot_cumulative(
                uyear, ureg, regional_ACE, 'ACE', '$10^4 kn^2$')

            self.plot_cumulative(
                uyear, ureg, regional_PDI, 'PDI', '$m^3 s^{-2}$')
    ...
    @staticmethod
    def accum_by_year_and_region(
            uyear, ureg, year, region_id, start_y, var, var_out):
    ...
    def plot_cumulative(self, uyear, ureg, var, var_name, units):
        ...
        while q < n_reg-3:
            ...
            ax.fill_between(
                uyear, bot, top, facecolor=fill_col[q], alpha=0.75, zorder=3)

            ax.plot(
                uyear, top, color=fill_col[q], linewidth=2, label=rnms[q],
                zorder=3)
