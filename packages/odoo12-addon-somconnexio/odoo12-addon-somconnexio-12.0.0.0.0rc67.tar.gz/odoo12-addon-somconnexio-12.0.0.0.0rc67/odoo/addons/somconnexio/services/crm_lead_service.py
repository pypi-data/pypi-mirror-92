...
class CRMLeadService(Component):
    ...
    def _prepare_create_isp_info(self, isp_info):
        ...
        if isp_info["type"] == "portability":
            if not isp_info.get("previous_owner_name"):
                owner = self.env['res.partner'].browse(self.partner_id)
                previous_owner_dict = {
                    "previous_owner_vat_number": owner.vat,
                    "previous_owner_name": owner.lastname,
                    "previous_owner_first_name": owner.firstname,
                }
                isp_info.update(previous_owner_dict)
