...
class Query:
    ...
    * A result object that is emitted once the query is run.
    ...
    def start_at(self, n: Union[str, Node, List[Node]]) -> 'Query':
        ...
        This sets the initial set of elements in the current query. The qu
        These queries
        :param n:
        :return:
        """
    ...
    def out(self, node_type: Union[str, Node, List[Node]], edge_type: Union[str, List[str]] = None) -> 'Query':
        """
    ...
    def emit_with_edges(self, emit_key: str, fields: List[str] = None) -> 'Query':
        """
        add the current collection with its outgoing edges to the output
        Args:
            emit_key:
            fields:
    ...
    def of_type(self, node_type: str) -> 'Query':
        """
        filters the current collection by type
    ...
    def of_types(self, node_types: str) -> 'Query':
        """
        filters the current collection by types
    ...
    def except_type(self, node_type: str) -> 'Query':
        """
        removes nodes from the current collection of the given type
    ...
    def except_types(self, node_types: List[str]) -> 'Query':
        """
        removes nodes from the current collection of the given types
    ...
    def is_related_to(self, n: Union[str, List[str], Node, List[Node]],
                      assume_bidirectional_edges: bool = True) -> 'Query':
    ...
    def is_not_related_to(self, n: Union[str, List[str], Node, List[Node]],
                          assume_bidirectional_edges: bool = True) -> 'Query':
        if isinstance(n, Node):
            self.query.append(
            ...
            if isinstance(n[0], str):
                ...
                self.query.append({"op": "IsNotRelatedTo",
                                   "args": {"nodes": n, "assumeBidirectionalEdges": assume_bidirectional_edges}})
            else:
                raise ValueError
        else:
            raise ValueError
