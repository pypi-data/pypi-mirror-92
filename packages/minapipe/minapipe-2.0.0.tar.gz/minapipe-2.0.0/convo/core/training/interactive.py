...
from convo.shared.nlu.constants import TEXT, INTENT_NAME_KEY
...
from convo.core.constants import DEFAULT_SERVER_FORMAT, DEFAULT_SERVER_PORT
...
from convo.shared.constants import INTENT_MESSAGE_PREFIX, DEFAULT_SENDER_ID, UTTER_PREFIX
from convo.shared.core.trackers import EventVerbosity, DialogueStateTracker
...
def format_bot_output(message: BotUttered) -> Text:
    ...
    if data.get("buttons"):
        output += "\nButtons:"
        choices = convo.cli.utils.button_choices_from_message_data(
    ...
    if data.get("elements"):
        ...
        for idx, element in enumerate(data.get("elements")):
            element_str = convo.cli.utils.element_to_string(element, idx)
            output += "\n" + element_str
...
def _slot_history(tracker_dump: Dict[Text, Any]) -> List[Text]:
    ...
    for k, s in tracker_dump.get("slots", {}).items():
        colored_value = convo.shared.utils.io.wrap_with_color(
            str(s), color=convo.shared.utils.io.bcolors.WARNING
        )
    ...
    questions = questionary.form(
        export_stories=questionary.text(
            ...
            validate=io_utils.file_type_validator(
                convo.shared.data.MARKDOWN_FILE_EXTENSIONS
                + convo.shared.data.YAML_FILE_EXTENSIONS,
        ...
        export_nlu=questionary.text(
            ...
            default=PATHS["nlu"],
            validate=io_utils.file_type_validator(
                list(convo.shared.data.TRAINING_DATA_EXTENSIONS),
...
def _write_stories_to_file(
    ...
    if convo.shared.data.is_likely_yaml_file(export_story_path):
    ...
    with open(
        export_story_path, append_write, encoding=convo.shared.utils.io.DEFAULT_ENCODING
    ) as f:
        i = 1
        for conversation in sub_conversations:
            parsed_events = convo.shared.core.events.deserialise_events(conversation)
...
def _write_nlu_to_file(export_nlu_path: Text, events: List[Dict[Text, Any]]) -> None:
    ...
    convo.shared.utils.io.write_text_file(stringified_training_data, export_nlu_path)


def _get_nlu_target_format(export_path: Text) -> Text:
    guessed_format = loading.guess_format(export_path)

    if guessed_format not in {MARKDOWN, CONVO, CONVO_YAML}:
        if convo.shared.data.is_likely_json_file(export_path):
            guessed_format = CONVO
        elif convo.shared.data.is_likely_markdown_file(export_path):
            guessed_format = MARKDOWN
        elif convo.shared.data.is_likely_yaml_file(export_path):
            guessed_format = CONVO_YAML
    ...
    collected_actions = list(
        {
            e["name"]
            for e in actions
            if e["name"] not in convo.shared.core.constants.DEFAULT_ACTION_NAMES
...
async def _predict_till_next_listen(
    ...
    if len(events) >= 2:
        ...
        if last_event.get("event") == BotUttered.type_name and last_event["data"].get(
            ...
            if user_selection != convo.cli.utils.FREE_TEXT_INPUT_PROMPT:
...
def _get_button_choice(last_event: Dict[Text, Any]) -> Text:
    ...
    return convo.cli.utils.payload_from_button_question(question)
...
async def _confirm_form_validation(
    ...
    if not validate_input:
        ...
        await send_event(
            ...
            conversation_id,
            {
                "event": convo.shared.core.events.LoopInterrupted.type_name,
...
async def train_agent_on_start(
    ...
    _interpreter = convo.core.interpreter.create_interpreter(
