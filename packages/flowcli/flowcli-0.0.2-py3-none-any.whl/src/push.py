...
def push(file):
    if file is None:
        is_exist = os.path.exists(os.getcwd() + '/tool.json')
        if is_exist == False:
...
def pushImage(config_json):
    if config_json['id'] == '':
        ...
        image.tag(common.get_docker_registryl() + '/' + path, tag='latest')
        auth = common.get(url=common.get_cloud_base_url() + '/tool/pass')
        for line in client.images.push(common.get_docker_registryl() + '/' + path, auth_config={'username': auth['account'], 'password': auth['password']}, stream=True, decode=True):
            if 'error' in line and line['error'] != '':
                print(line['error'])
                client.images.remove(common.get_docker_registryl() + '/' + path + ':latest')
