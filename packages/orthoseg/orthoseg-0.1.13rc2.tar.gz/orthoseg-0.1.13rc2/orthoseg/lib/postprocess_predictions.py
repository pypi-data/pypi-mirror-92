...
def postprocess_predictions(
    ...
    if dissolve:
        ...
        geofile.add_column(path=curr_output_path, 
                name='area', type='real', expression='ST_Area(geom)', force_update=True)
        geofile.add_column(path=curr_output_path, 
                name='nbcoords', type='integer', expression='ST_NumPoints(geom)', force_update=True)
    ...
    try:
        ...
        if prediction_cleanup_params is not None:
            ...
            if 'simplify_algorythm' in prediction_cleanup_params:
                ...
                geoms_gdf = geoms_gdf.loc[~geoms_gdf.is_empty] 
                if len(geoms_gdf) == 0:
                    return None
                geoms_gdf = geoms_gdf.loc[~geoms_gdf.isna()]  
                if len(geoms_gdf) == 0:
                    return None
