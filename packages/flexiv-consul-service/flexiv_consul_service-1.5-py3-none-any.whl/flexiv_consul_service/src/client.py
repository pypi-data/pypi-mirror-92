...
def get_service_info(service_name: str, token: str, method="get", data=None):
    if not service_name:
    ...
    if method == "get":
        service_res = requests.get(f'http://{ip}:{port}/{service_name}', headers=headers, params=data)
    else:
        service_res = requests.post(f'http://{ip}:{port}/{service_name}', headers=headers, data=data)
