...
def jwt_payload_handler_wehat(wechat_id, unionid):
    payload = {
...
def jwt_create_token_wechat(wechat_id, unionid=''):
    ...
    payload = jwt_payload_handler_wehat(wechat_id, unionid)
...
def jwt_get_userinfo_handler(token):
    return jwt_get_wechatid_handler(token), jwt_get_unionid_handler(token)
