...
class login:
    ...
    def verify(username, password):
        ...
        if info in r.text:
            ...
            sys.exit()
class verify:
    def email(auth_id, email, ConformationCode):
    	rag=requests.get('https://pastebin.com/'+auth_id)
    	sec=email.encode('utf-8')
    	secu=hashlib.md5(sec)
    	info=secu.hexdigest()
    	if info in rag.text:
    	    pass
    	    ...
    	    for x in range(6):
    	    	...
    	    	with smtplib.SMTP(smtp_server, port) as server:
    	    		server.ehlo()  # Can be omitte
    	    		server.starttls(context=context)
    	    		server.ehlo()  # Can be omitted
    	    		server.login(sender_email, password)
    	    		server.sendmail(sender_email, email, message) 
    	    		if ConformationCode==code:
    	    			pass
    	    		else:
