...
class PusherExecutor(Executor):
    ...
    def execute(
        ...
        assert (
            "is_single" in input_dict
            and len(input_dict["is_single"]) == 1
        )
        is_single = input_dict["is_single"][0].meta.uri
        if is_single != "single":
            raise RuntimeError("model should be converted by single worker")
