...
def print_warehouse_rules(line, user_ns):
    ...
    for location, routes in list(result.items()):
        print((' ' * 0, location))
        for route, rules in list(routes.items()):
            print((' ' * 4, route))
            for rule in rules:
                print((' ' * 8, rule))
            print()
...
def format_sequence_record(record):
    return '{: >2}. {}'.format(record.sequence, format_record(record))
...
def print_warehouse_rules_table(line, user_ns):
    ...
    locations = list(list(result.values())[0].keys())
