...
def macd(close, n_fast=12, n_slow=26, n_sign=9, fillna=False):
    ...
    """
    [macd, macdsignal, macdhist] = talib.MACD(close.values, fastperiod=n_fast, slowperiod=n_slow, signalperiod=n_sign)

#    emafast = talib.EMA(close, n_fast, fillna)
#    emaslow = talib.EMA(close, n_slow, fillna)
#    macd = emafast - emaslow
#    if fillna: macd = macd.replace([np.inf, -np.inf], np.nan).fillna(0)
...
#        macd_signal = macd_signal.replace([np.inf, -np.inf], np.nan).fillna(0)
    return pd.Series(macdsignal, name='MACD_sign')
...
#        macd_diff = macd_diff.replace([np.inf, -np.inf], np.nan).fillna(0)
    return pd.Series(macdhist, name='MACD_diff')
...
#    pdn = low.combine(cs,  lambda x1, x2: min(x1, x2) if np.isnan(x1) == False and np.isnan(x2) == False else np.nan)
    pdm = np.maximum(high,cs)
    pdn = np.minimum(low,cs)
...
def vortex_indicator_pos(high, low, close, n=14, fillna=False):
    ...
    """
    cs=close.shift(1)
    tr=np.maximum(high,cs) - np.minimum(low,cs)
...
def trix(close, n=15, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    trix=talib.TRIX(close, timeperiod=n)
#    ema1 = talib.EMA(close)
#    ema2 = talib.EMA(ema1)
#    ema3 = talib.EMA(ema2)
...
def mass_index(high, low, n=9, n2=25, fillna=False):
    ...
    """
    amplitude = high - low
    ema1 = talib.EMA(amplitude, n)
    ema2 = talib.EMA(ema1, n)
    mass = ema1 / ema2
