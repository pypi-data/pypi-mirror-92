...
for setting in model_settings:
    ...
    # When mlops is a package, deploying a folder (required in order to attach
    # model files) requires at least copying the handler file. This is
    # challenging because the service setting file is not created yet, so we
    # can't import the handler directly
    ...
    # File name can't be mlops_settings.py, or there will be an importing error
    # when the service test is run (user settings <- model test <- service test)
    ...
    if model_test_file:
        ...
        logging.warning("Model test file is missing! Didn't run service tests")
    ...
    if ENV != "dev" or LOCAL_DEPLOYMENT:
        ...
        if deployment_platform == 'cdf':
            ...
            # Dockerfile has to be at the top (gcloud builds)
            shutil.move(
                os.path.join('mlops', 'gc', 'Dockerfile'), 
                'Dockerfile'
            )
        else:
