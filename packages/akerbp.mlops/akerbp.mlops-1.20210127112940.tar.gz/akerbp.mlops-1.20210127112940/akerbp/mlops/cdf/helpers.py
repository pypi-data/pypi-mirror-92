...
def set_up_cdf_client(api_keys=api_keys):
    """
    ...
    Input:
      - api_keys: dictionary with api keys. Expected labels contain 
      "functions", "data" and/or "files".
    """
    if not api_keys:
        raise Exception("Empty api keys")

    for k,v in api_keys.items():
        if "functions" in k:
            global_client["functions"] = get_client(v)
        elif "data" in k:
            global_client["data"] = get_client(v)
        elif "files" in k:
            global_client["files"] = get_client(v)
        else:
            raise Exception("Unknown CDF api key label")
