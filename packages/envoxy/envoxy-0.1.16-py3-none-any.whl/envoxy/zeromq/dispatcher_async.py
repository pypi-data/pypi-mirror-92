...
from ..utils.singleton import Singleton, Singleton2
...
class ZMQ(Singleton2):
    ...
    def __init__(self):
        ...
        for _server_key in self._server_confs.keys():
            ...
            self._instances[f'{_server_key}_{self.thread_name}'] = {
            ...
            self.connect(self._instances[f'{_server_key}_{self.thread_name}'])
    ...
    def connect(self, instance):
        ...
        _socket.identity = str('{}_{}'.format(uuid.uuid4(), self.thread_name)).encode('ascii')

        instance[f'socket_{self.thread_name}'] = _socket
        ...
        for _port in self.port_range(instance):
            ...
            try:
                ...
                instance[f'socket_{self.thread_name}'].connect(_url)

                if not instance[f'socket_{self.thread_name}'].closed:
        ...
        if len(_connected) > 0:
            Log.trace(f">>> Successfully connected to ZEROMQ machine: {', '.join(_connected)} {self.thread_name}")
            Log.trace(f">>Registering new socket {instance[f'socket_{self.thread_name}']} {self.thread_name}")
            self._poller.register(instance[f'socket_{self.thread_name}'], zmq.POLLIN)
    ...
    def disconnect(self, instance):
        ...
        try:
            ...
            Log.trace(f"Disconnecting {instance[f'socket_{self.thread_name}']} {self.thread_name}")
            instance[f'socket_{self.thread_name}'].close()

            if instance[f'socket_{self.thread_name}'].closed:
                Log.trace('>>> Successfully disconnected to ZEROMQ machine: {}'.format(f"{instance[f'socket_{self.thread_name}']} {self.thread_name}"))
                self._poller.unregister(instance[f'socket_{self.thread_name}'])

                return True

        except Exception as e:
            Log.error(f'>>Exception during DISCONNECT {e} {self.thread_name}')
            return False
    ...
    def restore_socket_connection(self, server_key, force_new_socket=False):
        ...
        try:
            ...
            if self._instances[f'{server_key}_{self.thread_name}']['retries_left'] > 0:

                # Socket is confused. Close and remove it.
                _result = self.disconnect(self._instances[f'{server_key}_{self.thread_name}'])

                self._instances[f'{server_key}_{self.thread_name}']['retries_left'] -= 1

                Log.warning(f"ZMQ::restore_socket_connection : Reconnecting and resending: {self._instances[server_key]['url']} retry #{self._instances[server_key]['retries_left']} {self.thread_name}")

                if force_new_socket:  # Create new connection

                    del self._instances[f'{server_key}_{self.thread_name}'][f'socket_{self.thread_name}']

                    _socket = self._context.socket(zmq.REQ)

                    self._instances[f'{server_key}_{self.thread_name}'][f'socket_{self.thread_name}'] = _socket

                self.connect(self._instances[f'{server_key}_{self.thread_name}'])

                if dict(self._poller.poll(ZEROMQ_RETRY_TIMEOUT)):
                    _restore_successful = True

        except Exception as e:
            Log.error(f'>>Exception during RESTORE {e} {self.thread_name}')

        if self._instances[f'{server_key}_{self.thread_name}']['retries_left'] == 0 and not _restore_successful:
            Log.alert(f"ZMQ::send_and_recv : Server seems to be offline, abandoning: {self._instances[server_key]['url']} {self.thread_name}")
            self._instances[f'{server_key}_{self.thread_name}']['retries_left'] = ZEROMQ_REQUEST_RETRIES
    ...
    def send_and_recv(self, server_key, message):
        _response = None
        _instance = self._instances[f'{server_key}_{self.thread_name}']

        try:

            while _instance['retries_left']:

                _instance[f'socket_{self.thread_name}'].send_string('', zmq.SNDMORE)
                _instance[f'socket_{self.thread_name}'].send_string(json.dumps(message))

                Log.info('{} IN {} {}'.format(message, _instance[f'socket_{self.thread_name}'], self.thread_name))
                ...
                try:
                    ...
                    while True:
                        ...
                        # Log.info(_socks)
                        # Log.info(_socks.get(_instance[f'socket_{self.thread_name}']))
                        # Log.info(_socks.get(_instance[f'socket_{self.thread_name}']) == zmq.POLLIN )

                        if not _socks:
                            raise NoSocketException(f'No events received in {ZEROMQ_POLLIN_TIMEOUT/1000} secs on {_instance["url"]} {self.thread_name}')


                        if _socks.get(_instance[f'socket_{self.thread_name}']) == zmq.POLLIN:
                            # Log.trace('valid socket')

                            _response = _instance[f'socket_{self.thread_name}'].recv() # discard delimiter
                            # Log.trace(_response)

                            _response = _instance[f'socket_{self.thread_name}'].recv_json() # actual message
                            # Log.trace(_response)
                            ...
                            Log.info('Returning response {} {}'.format(_response, self.thread_name))
                    ...
                    Log.error(f"ZMQ::send_and_recv : Could not connect to ZeroMQ machine: {_instance['url']} {self.thread_name}")

                    if not self.restore_socket_connection(server_key, force_new_socket=True):
                        break
    
        except NoSocketException as e:
            Log.warning(f"ZMQ::send_and_recv : It is not possible to send message using the ZMQ server \"{_instance['url']}\". Error: {e}  {self.thread_name}")
    ...
    def find_ports(self, server_key, file_path):
        ...
        try:
            ...
            with open(file_path) as ports_file:
                ...
                Log.notice(f"{len(services)} entries found for {server_key} {self.thread_name}")
