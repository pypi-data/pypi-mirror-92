...
class GULA (Algorithm):
    ...
    @staticmethod
    def fit(dataset, targets_to_learn=None, supported_only=False): #variables, values, transitions, conclusion_values=None, program=None): #, partial_heuristic=False):
        """
        ...
        for var_id, (var_name, var_domain) in enumerate(dataset.targets):
            ...
            for val_id, val_name in enumerate(var_domain):
                ...
                rules += GULA.fit_var_val(feature_domains, var_id, val_id, negatives, positives) #variables, values, var, val, negatives, program)#, partial_heuristic)
    ...
    @staticmethod
    def fit_var_val(feature_domains, variable, value, negatives, positives=None): #variables, values, variable, value, negatives, program=None):#, partial_heuristic=False):
        """
