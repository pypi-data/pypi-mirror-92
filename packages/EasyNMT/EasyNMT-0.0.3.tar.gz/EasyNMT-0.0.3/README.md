...
- Smart batching for optimal translation speed

At the moment, we provide interfaces for two architectures: [Opus-MT](#Opus-MT) from [Helsinki-NLP](https://github.com/Helsinki-NLP/Opus-MT) and [M2M_100](#M2M_100) from [Facebook Research](https://arxiv.org/abs/2010.11125).
...
For an example how to use EasyNMT, see: [EasyNMT Google Colab Example](https://colab.research.google.com/drive/1X47vgSiOphpxS5w_LPtjQgJmiSTNfRNC?usp=sharing)
...
print(model.translate(sentences, source_lang='en', target_lang='de'))
```
...
print(model.translate(document, source_lang='en', target_lang='de'))
```
...
### Automatic Language Detection
So far, we have set `source_lang` for the `translate`  methods. If you do not set this parameter, automatic language detection is done using [fastText](https://fasttext.cc/blog/2017/10/02/blog-post.html). This also allows you to provide a list with sentences / documents that have various languages:
...
Currently, 4 models are available which support languages between any of more than 100 languages.
...
```python
from easynmt import EasyNMT
model = EasyNMT('m2m_100_418M')   #or: asyNMT('m2m_100_1.2B') 
```
