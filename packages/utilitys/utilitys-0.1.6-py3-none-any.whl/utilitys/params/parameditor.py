...
class ParamEditor(QtWidgets.QDockWidget):
  ...
  def __init__(self, parent=None, paramList: t.List[t.Dict] = None, saveDir: FilePath=None,
    ...
    self.tree = flexibleParamTree(topParam, showTop=False)
  ...
  def formatFileName(self, stateName: t.Union[str, Path]=None):
    ...
    if self.saveDir is None and not stateName:
      # Prevents ValueError at the final return
      return Path(f'.{self.fileType}')
    loadName = Path(stateName)
    if self.saveDir is not None and not loadName.is_absolute():
      loadName = self.saveDir/loadName
    return loadName.with_suffix(f'.{self.fileType}')
