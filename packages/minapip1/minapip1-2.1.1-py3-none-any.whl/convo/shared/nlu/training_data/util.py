...
def get_file_format_extension(resource_name: Text) -> Text:
    ...
    if not file_formats:
        return convo.shared.data.yaml_file_extension()

    known_file_formats = {
        loading.MARKDOWN: convo.shared.data.markdown_file_extension(),
        loading.CONVO_YAML: convo.shared.data.yaml_file_extension(),
    }
    fformat = file_formats[0]
    if all(f == fformat for f in file_formats):
        return known_file_formats.get(fformat, convo.shared.data.yaml_file_extension())
