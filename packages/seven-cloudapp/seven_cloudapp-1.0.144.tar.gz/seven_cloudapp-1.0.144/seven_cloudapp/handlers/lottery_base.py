...
:LastEditTime: 2021-01-19 10:48:12
...
class LotteryBaseHandler(SevenBaseHandler):
    ...
    def start_turntable(self, app_id, open_id, act_id, login_token):
        ...
        if TimeHelper.format_time_to_datetime(now_date) < TimeHelper.format_time_to_datetime(act_info.start_date):
            log_info["code"] = "NoAct"
            log_info["message"] = "活动将在" + act_info.start_date + "开启"
            return log_info
        ...
        if user_info.login_token != login_token:
            ...
            return log_info
        if user_info.lottery_value - act_info.lottery_value < 0:
            log_info["code"] = "NoNum"
            log_info["message"] = "对不起,积分不足" if act_info.currency_type == 2 else "对不起,次数不足"
            return log_info
        ...
        try:
            ...
            #扣除用户次数
            user_info.lottery_value -= act_info.lottery_value
            user_info.lottery_sum += 1
            user_info_model.update_table(f"lottery_value=lottery_value-{act_info.lottery_value},lottery_sum=lottery_sum+1", "id=%s", params=user_info.id)
            ...
            lottery_value_log.log_title = "大转盘抽奖-" + str(act_info.lottery_value) + last_content
        ...
        if log_info["code"] == "Success":
            BehaviorModel().report_behavior_log(app_id, act_id, open_id, act_info.owner_open_id, 'LotteryAddUserCount', 1)
            BehaviorModel().report_behavior_log(app_id, act_id, open_id, act_info.owner_open_id, 'LotteryUserCount', 1)
            BehaviorModel().report_behavior_log(app_id, act_id, open_id, act_info.owner_open_id, 'LotteryCount', 1)
        return log_info
