...
class OAPDash(dash.Dash):
    ...
    def index(self, *args, **kwargs):  # pylint: disable=unused-argument
        ...
        # check auth
        url = 'https://asura.opsramp.net/msps.json?_ajax=true'
        res = requests.get(url, verify=False, cookies=flask.request.cookies)
        if res.status_code !=200 or 'login_password' in res.text:
            # TODO: get redirect url from flask.request.url
            redirect_url = '/metered-usage-insight'
            return flask.redirect(f"https://localhost/tenancy/web/login?cb={redirect_url}", code=302)
