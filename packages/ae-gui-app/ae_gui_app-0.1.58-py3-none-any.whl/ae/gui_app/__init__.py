...
from ae.base import NAME_PARTS_SEP, now_str, snake_to_camel                             # type: ignore
from ae.paths import Collector, norm_path, path_folders                                 # type: ignore
...
class MainAppBase(ConsoleApp, ABC):
    ...
    def backup_config_resources() -> str:   # pragma: no cover
        ...
        coll = Collector()
        coll.collect("{app}", "{usr}", "{cwd}", append=("*.ini", "*.cfg"))
        for file in coll.files:
            if not os.path.exists(os.path.basename(file)):  # don't overwrite the first one found (the active one)
                shutil.copy2(file, backup_root)
        ...
        for path in coll.paths:
            shutil.copytree(path, os.path.join(backup_root, os.path.basename(path)))
    ...
    # application events - declared in the order they get executed/fired
