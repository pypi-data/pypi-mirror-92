...
from typing import Type, Generator, get_args, Dict, ClassVar, Any, Tuple
...
class Manager:
    ...
    def query(self, **kwargs) -> datastore.query.Iterator:
        # base query
...
class Metadata:
    ...
    def from_dict(self, data: Dict) -> EmbeddedDocument:
        ...
        def _build(klass, value):
            ...
            if issubclass(klass, EmbeddedDocument):
                return klass.Meta.from_dict(data=data)
    ...
    def to_dict(self, obj: EmbeddedDocument) -> dict:
...
class ORM(type):
    def __new__(cls, name, bases, attrs):
        ...
        if not is_abstract_model:
            ...
            if DEFAULT_PK_FIELD not in attrs['__annotations__']:
                attrs['__annotations__'][DEFAULT_PK_FIELD] = str
...
class EmbeddedDocument(metaclass=ORM):
    ...
    def deserialize(cls, **kwargs) -> EmbeddedDocument:
        return cls.Meta.from_dict(**kwargs)
