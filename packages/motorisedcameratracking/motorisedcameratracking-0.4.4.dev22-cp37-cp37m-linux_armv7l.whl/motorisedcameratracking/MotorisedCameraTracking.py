...
class MotorisedCameraTracking:
    ...
    def track(self,target: str,options: dict = {}):
        ...
        if self.checkTargetSupported(target):#checks the target is supported
            ...
            self.p1.start()
            self.p2 = threading.Thread(target=self.MC.main,args=(self.dataQueue, self.controlQueueMC,))
            self.p2.start()
    ...
    def trackLimited(self, target: str, xLimit1: float = 0, xLimit2: float = 0, yLimit1: float = 0, yLimit2: float = 0, options: dict = {}):
        ...
        if self.checkTargetSupported(target):
            a=Imaging(self.MC,self.dataQueue,self.controlQueueImg,target,camera=self.camera,mode=self.config['imagingMode'],extras={'xMaxSpeed':xMaxSpeed,'yMaxSpeed':yMaxSpeed})
    ...
    def getFrameAsImage(self,resolution: list,includeVelocity: bool = True,includeTime: bool = False):
