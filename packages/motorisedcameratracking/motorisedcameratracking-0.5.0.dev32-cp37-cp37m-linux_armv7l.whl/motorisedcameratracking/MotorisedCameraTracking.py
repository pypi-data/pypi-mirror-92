...
class MotorisedCameraTracking:
    ...
    def checkLimits(self, xLimit1, xLimit2, yLimit1, yLimit2):
        if type(xLimit1) == int and type(xLimit2) == int and type(yLimit1) == int and type(yLimit2) == int:
            if xLimit1<0 and xLimit2>0 and xLimit1<0 and yLimit2>0:
                return True
    ...
    def terminate(self):
        ...
        if self.running == True:#checks running is true as otherwise an error will occur from attempting is_alive() on a none object
            ...
            while True:#loops as the processes have to finish what they are doing
                if not (self.p1.is_alive()):# or self.p2.is_alive()):#waits for them to be finished
    ...
    def setSpecsZero(self,waitTime,speed1,speed2):
        ...
        if not (type(waitTime)==Float and type(speed1)== float and type(speed2)==float):
            raise ValueError("incorrect arguments given")

        if waitTime>0 and speed1>0 and speed2>0:
