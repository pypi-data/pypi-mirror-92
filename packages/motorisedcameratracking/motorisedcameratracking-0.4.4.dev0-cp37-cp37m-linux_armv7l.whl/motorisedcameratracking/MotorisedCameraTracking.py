...
class MotorisedCameraTracking:
    ...
    def getFrameAsImage(self,resolution: list,includeVelocity: bool = True,includeTime: bool = True):
        ...
        dataString=""
        if includeVelocity:
            dataString=("The x velocity is: "+self.MC.getXVelocity()+". The y velocity is: "+self.MC.getYVelocity()+". ")
        if includeTime:
            dataString+=("The Time is: "+time.asctime(time.localtime(time.time()))+".")
        if includeVelocity or includeTime:
            image=cv2.putText(image,dataString)
