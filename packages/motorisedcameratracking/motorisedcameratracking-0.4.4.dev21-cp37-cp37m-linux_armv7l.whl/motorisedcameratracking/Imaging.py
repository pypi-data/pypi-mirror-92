...
class Imaging:
    ...
    def mainLimited(self,xLimit1,xLimit2,yLimit1,yLimit2):#only supports one mode currently
...
###############################################################
    ...
    def intermediateTrackingLimited(self,xLimit1,xLimit2,yLimit1,yLimit2):
        ...
        while True:
            ...
            targetsList=self.OR.getCoordinates(img)#gets the coordinates of the objects
            if len(targetsList[0]) == 1:#checks the coordinates are not none
                xCo=targetsList[0][1]
                yCo=targetsList[0][2]
                if not self.inCentralBox(xCo,yCo):
                    xV,yV=self.intermediateVelocityCalculation(xV,yV,xCo,yCo)
                    self.dataQueue.put([xV,yV])#only sends a new velocity when one is available
            elif self.isMultiple(targetList):
                pass#maintains the same velocity
                #tweak the tracking for the scenario
    ...
    def outsideLimits(self,xLimit1,xLimit2,yLimit1,yLimit2):
        if not (xLimit1<self.MC.getXDisplacement()<xLimit2 or yLimit1<self.MC.getXDisplacement()<yLimit2):
            return True
        else:
            return False
