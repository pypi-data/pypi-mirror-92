...
def osem_cone_beam_reconstruction_filter_init_docstring():
    ...
    if isinstance(itk.OSEMConeBeamReconstructionFilter, itkTemplate.itkTemplate):
        filter_object = itk.OSEMConeBeamReconstructionFilter.values()[0]
    else:
        filter_object = itk.OSEMConeBeamReconstructionFilter

    osem_cone_beam_reconstruction_filter.__doc__ = filter_object.__doc__
    osem_cone_beam_reconstruction_filter.__doc__ += "\n Args are Input(s) to the filter.\n"
    osem_cone_beam_reconstruction_filter.__doc__ += "\n Available Keyword Arguments:\n"
    if isinstance(itk.OSEMConeBeamReconstructionFilter, itkTemplate.itkTemplate):
        osem_cone_beam_reconstruction_filter.__doc__ += itkHelpers.filter_args(filter_object)[0]
        osem_cone_beam_reconstruction_filter.__doc__ += "\n"
        osem_cone_beam_reconstruction_filter.__doc__ += itkHelpers.filter_args(filter_object)[1]
    else:
        osem_cone_beam_reconstruction_filter.__doc__ += "".join([
