...
class RequestClient:
    ...
    def run_request(self, method, url, query_params=None, headers=None,
        ...
        if r.status == 404:
            raise ApiException(r.status,'Error 404. Not found')

        if r.status == 401:
            raise ApiException(r.status,"Error 401. Cant authorize. Check your api token")

        if r.status == 500:
            raise ApiException(r.status,"Internal error",http_resp=r)

        if r.status==400:
            raise ApiException(r.status,"Error 400. Bad request:",http_resp=r)
