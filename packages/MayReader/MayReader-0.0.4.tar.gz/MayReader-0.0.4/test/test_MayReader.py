...
import unittest
from MayReader.MayReader import MayReader


class TestMayReader(unittest.TestCase):
    
    def test_isComment(self):
        self.assertTrue(MayReader.isComment("//This is comment"))
        self.assertFalse(MayReader.isComment("/This is not a comment"))
               
        
    def test_isNode(self):
        self.assertTrue(MayReader.isNode("createNode transform -s -n"))
        self.assertFalse(MayReader.isNode("/createNode transform -s -n"))
        
    def test_outputWithWrongInputFiile(self):
        mr = MayReader.MayReader("notRealFile.txt")
        self.assertFalse(mr.getMashObjects())
        
    def test_outputWithProperInput(self):
        mr = MayReader.MayReader("example_file_1.ma")
