...
def delete_items_by_hashkey(dynamodbTableRes,hashkey):
    ...
    for ddbkey in dynamodbTableRes.key_schema:
    ...
    if hashKeyName is not None:
        ...
        result = get_records_by_key_condition(tablename = "",KeyConditionExpression= KeyConditionExpression,dynamodbTableRes = dynamodbTableRes)



        # Delete items in batch - Include rangeKey if required
        with dynamodbTableRes.batch_writer() as batch:
