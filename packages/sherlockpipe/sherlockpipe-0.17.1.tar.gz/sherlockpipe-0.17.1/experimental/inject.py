...
model = tls.transitleastsquares(time, flux0)
results0 = model.power(M_star=mass, M_star_max=mstar_max, M_star_min=mstar_min, R_star=radius,
                      R_star_min=rstar_min, R_star_max=rstar_max,
                      period_min=0.4, period_max=7)
mask0 = transit_mask(time, results0.period, results0.duration, results0.T0)
time0 = time[~mask0]
flux1 = flux0[~mask0]
flux_err1 = flux_err[~mask0]
model = tls.transitleastsquares(time0, flux1)
results1 = model.power(M_star=mass, M_star_max=mstar_max, M_star_min=mstar_min, R_star=radius,
                      R_star_min=rstar_min, R_star_max=rstar_max,
                      period_min=0.4, period_max=7)
mask1 = transit_mask(time, results1.period, results1.duration, results1.T0)
...
dir = "/home/pozuelos/martin/ir/toi2096/curves"
if not os.path.isdir(dir):
    os.mkdir(dir)
max_period = 40
min_period = 0.5
for period in np.arange(min_period, max_period, 1.0):
    for t0 in np.arange(time[60], time[60] + period - 0.1, period / 4):
        for rplanet in np.arange(4, 0.65, -0.2):
            ...
            if len(time_model) > 0:
                time_model = time_model[~mask0 & ~mask1]
                flux_model = flux_model[~mask0 & ~mask1]
                flux_err_model = flux_err_model[~mask0 & ~mask1]
