...
class SnowflakeDataWarehouse(DataWarehouse, Session, metaclass=DataWarehouseSession):
    ...
    @track_usage
    def get_source_columns(self, table: Optional[str] = None, data_type: Optional[str] = None):
        filters = "WHERE 1=1"
        filters += f" AND TABLE_NAME='{table}'" if table else ""
        filters += f" AND DATA_TYPE='{data_type}'" if data_type else ""
        result_set = self.execute_query(f"SELECT * FROM RASGO_DATA_SOURCE_COLUMNS {filters}")
        return pd.DataFrame.from_records(iter(result_set), columns=[x[0] for x in result_set.description])
    ...
    @track_usage
    def grant_table_ownership(self, table: str, role: str):
        self.execute_query(f"GRANT OWNERSHIP ON {table} TO ROLE {role} REVOKE CURRENT GRANTS;")
    ...
    def _make_table_metadata(self, creds: dict, table: str):
        if table.count(".") > 0:
            table = table.split(".")[-1]
        metadata = {
