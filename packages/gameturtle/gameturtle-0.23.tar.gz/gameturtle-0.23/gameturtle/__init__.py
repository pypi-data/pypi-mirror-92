"""
   gameturtle.py
   本模块的创造过程在本人编写的《Python海龟宝典》下册原理篇中(0.22版)。
   
   0.23版本的gameturtle模块有如下更新:
   1、新增turtleimage单独函数，实例化时不写frames参数时，会自动生成海龟图。
   2、把Rect类包含进来了。
   3、新增GameTurtle类的ondrag方法。
   4、颜色碰撞可以用字符串表示颜色了(ImageColor.getrgb转换的)
"""
...
from PIL import Image,ImageTk,ImageGrab,ImageOps,ImageDraw,ImageColor
...
class GameTurtle:
    ...
    def __init__(self,canvas,frames=None,pos=None,visible=True,heading=0,tag='sprite'):
        ...
        self._start_nextshape_time = time.time()# 下一个造型start时间
        self._start_preshape_time = time.time() # 上一个造型start时间
        self._shapeinterval = 0.1               # 造型更新间隔
    ...
    def nextshape(self):
        """下一个造型"""         
        if time.time() - self._start_nextshape_time >= self._shapeinterval:
            self._start_nextshape_time = time.time()
    ...
    def previousshape(self):
        """上一个造型"""       
        if time.time() - self._start_preshape_time >= self._shapeinterval:
            self._start_preshape_time = time.time()
            self._shapeindex -= 1                       # 索引号加1
            self._shapeindex %= self._shape_amounts     # 对数量求余
            self._process_shape()
            self._setshape()                            # 配置造型
...
if __name__ == "__main__":
    ...
    jack = Sprite(cv,turtleimage('red'))
