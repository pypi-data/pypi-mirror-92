...
class Loss(Function):
    ...
    def CrossEntropy(self):
        Cross_Loss = MSE((self.Y), (self.Y_hat), self.layer_cache, self.layer_weights, self.layer_bias)
        return Cross_Loss
...
class CrossEntropy(Loss):
    def forward_pass(self):
        log_Y_hat = -np.log([self.Y_hat[i, self.Y[i]] for i in range(len(self.Y_hat))])
    ...
    def local_gradient(self):
        ...
        self.grad = {'dL': (self.Y_hat - ones) / float(len(self.Y))}
        return self.grad
