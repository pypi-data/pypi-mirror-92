...
def appendPronouns(merged, ranges, text, prefixwords):
    if len(prefixwords) == 0:
        return ranges
    prefixes = "|".join(["(\s|^)"+stop+"(\s|$)" for stop in prefixwords])
    for i in range(len(merged)):
        match = re.search("("+prefixes+")", text[merged[i][0]:merged[i][1]])
        if not match:
            continue
        start = match.start() + merged[i][0]
        end = len(text)
        if i < len(merged)-1:
            end = merged[i+1][0]

        # must have text after it...
        if end != merged[i][1]:
...
def wildgram(text, stopwords=STOPWORDS, topicwords=TOPICWORDS, include1gram=False, joinerwords=JOINERWORDS, prefixwords=[]):
    ...
    ranges = appendPronouns(merged, ranges, text, prefixwords)
