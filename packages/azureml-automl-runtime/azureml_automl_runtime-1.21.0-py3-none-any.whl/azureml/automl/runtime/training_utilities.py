...
from azureml.automl.core.shared.constants import NumericalDtype, TelemetryConstants, Tasks
...
def _get_subsampled_data_for_streaming(
    ...
    Validation.validate_type(training_data, "training_data", expected_types=dprep.Dataflow)
    Validation.validate_value(automl_settings.label_column_name, "label_column_name")

    target_column_name = cast(str, automl_settings.label_column_name)
    validation_size = automl_settings.validation_size or 0.0
    ...
    try:
        ...
        if validation_data is not None:
            ...
            _validate_train_valid_datatypes(training_data, validation_data)

        X, y, sample_weight = _materialize_data_for_validation(
            training_data, target_column_name, weight_column_name)
        ...
        if validation_size == 0 and validation_data is not None:
            X_valid, y_valid, sample_weight_valid = _materialize_data_for_validation(
                training_data, target_column_name, weight_column_name)
...
def _materialize_data_for_validation(
        dataset: dprep.Dataflow, target_column_name: str, weight_column_name: Optional[str],
        count: int = LargeDatasetLimit.VALIDATION_SUBSAMPLE_SIZE
    ...
    For cases when data is remote and potentially large (e.g., streaming scenario), this method will only load a fixed
    (usually small) number of records and materialize them into an appropriate format for each of the components of
    the data. X is returned as a pandas DataFrame, while y and weights are returned as a numpy array.

    :param dataset: Pointer to the data
    :param target_column_name: The name of the column to predict
    :param weight_column_name: The name of the column that represents the weights to be assigned to each sample
    ...
    """
    Contract.assert_type(dataset, "dataset", expected_types=dprep.Dataflow)
    Contract.assert_value(target_column_name, "target_column_name")

    non_training_columns = [target_column_name]
    if weight_column_name:
        non_training_columns.append(weight_column_name)

    dataflow_X = dataset.drop_columns(non_training_columns)
    dataflow_y = dataset.keep_columns(target_column_name)
    dataflow_weights = dataset.keep_columns(weight_column_name) if weight_column_name else None

    # todo move to validators, perhaps tabular data validator constructor?
    _validate_indistinct_target_column(dataflow_X, dataflow_y)

    materialized_X = dataflow_X.take(LargeDatasetLimit.VALIDATION_SUBSAMPLE_SIZE).to_pandas_dataframe()

    Contract.assert_true(
        len(dataflow_y.head(1).columns) == 1,
        "Expected target column to be a single column, got: {}".format(len(dataflow_y.head(1).columns))
    )
    materialized_y = dataflow_y.take(count).to_pandas_dataframe().iloc[:, 0].values
    ...
    if dataflow_weights:
        ...
        materialized_weights = dataflow_weights.take(count).to_pandas_dataframe().iloc[:, 0].values

    return materialized_X, materialized_y, materialized_weights


def _validate_indistinct_target_column(X: dprep.Dataflow, y: dprep.Dataflow) -> None:
...
def _validate_train_valid_datatypes(training_data: dprep.Dataflow, validation_data: dprep.Dataflow) -> None:
    """
    ...
    for obj, obj_name in [(training_data, "training_data"), (validation_data, "validation_data")]:
        Contract.assert_type(obj, obj_name, expected_types=dprep.Dataflow,
                             reference_code=ReferenceCodes._STREAMING_INVALID_DATATYPE)
...
def _get_raw_experiment_data(data_dict: Dict[str, Any], automl_settings: AutoMLBaseSettings) -> RawExperimentData:
    ...
    if automl_settings.enable_streaming:
        # overwrite X/y/.. with a sub-sampled and materialized version of the dataset
        X, y, X_valid, y_valid, weights, weights_valid = _get_subsampled_data_for_streaming(
            training_data, automl_settings, validation_data)
