...
def orchestrate_featurization(
    ...
    if automl_settings.enable_streaming:
        ...
        td_ctx = data_transformation.complete_featurization_timeseries(
            raw_data_context,
            td_ctx,
            experiment_observer,
            verifier
        ...
        data_transformation._create_cv_splits_transformed_data(
            td_ctx,
            raw_data_context,
            X,
            y,
        ...
        if not x_is_sparse and not skip_featurization(raw_data_context.featurization):
            # Only featurize if featurization was enabled and X is not sparse.
            td_ctx = data_transformation.complete_featurization(
                raw_data_context,
        ...
        if td_ctx._is_cross_validation_scenario():  # type: ignore
            # Refit transformers
            ...
            raw_X = _data_transformation_utilities._add_raw_column_names_to_X(
                ...
                None)
            td_ctx._refit_transformers(raw_X, y)  # type: ignore
