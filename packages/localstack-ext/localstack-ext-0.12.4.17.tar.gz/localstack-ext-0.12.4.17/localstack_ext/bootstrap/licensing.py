...
def read_api_key():
 ...
 if key:
  return key
 raise rDTxt('Unable to retrieve API key. Please configure $LOCALSTACK_API_KEY in your environment')
def fetch_key():
 ...
 try:
  ...
  result=requests.post('%s/activate'%config.API_URL,json.dumps(data),verify=rDTxC)
...
def cache_key_locally(api_key,key_b64):
 ...
 timestamp=rDTxH(rDTxb(now_utc()))
 key_raw=to_str(base64.b64decode(key_b64))
 for i in rDTxu(rDTxY(timestamp)):
  key_raw=str_insert(key_raw,i*2,timestamp[i])
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 configs['cached_key']={'timestamp':rDTxb(timestamp),'key_hash':md5(api_key),'key':key_b64}
...
def load_cached_key(api_key):
 ...
 if cached_key.get('key_hash')!=md5(api_key):
  raise rDTxt('Cached key was created for a different API key')
 now=now_utc()
 if(now-cached_key['timestamp'])>MAX_KEY_CACHE_DURATION_SECS:
  raise rDTxt('Cached key expired')
 timestamp=rDTxH(cached_key['timestamp'])
