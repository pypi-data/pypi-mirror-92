...
class CloudPodManager(xbJGB):
 BACKEND='_none_'
 CONFIG_FILE='.localstack.yml'
 def __init__(self,pod_name=xbJGw,config=xbJGw):
  self.pod_name=pod_name
  self.config=config
 def push(self):
  raise xbJGW('Not implemented')
 def pull(self):
  raise xbJGW('Not implemented')
 ...
 def get(cls,pod_name,config=xbJGw):
  ...
  raise xbJGW('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 @xbJGt
 def load_config(cls,safe=xbJGL):
  try:
   return yaml.load(to_str(load_file(cls.CONFIG_FILE)))
  except xbJGW:
   if safe:
    return{}
   raise xbJGW('Unable to find and parse config file "%s"'%cls.CONFIG_FILE)
 @xbJGt
 def data_dir(cls):
  ...
  if not config.DATA_DIR:
   raise xbJGW('Working with local cloud pods requires $DATA_DIR configuration')
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b'HEAD' not in repo or repo.refs.allkeys()==xbJGQ([b'HEAD'])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=xbJGe)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise xbJGW('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get('url')
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=xbJGH(self,'clone_dir',xbJGw)
...
def store_pod_metadata(pod_name,metadata):
 ...
 if response.status_code>=400:
  raise xbJGW('Unable to store pod metadata in API (code %s): %s'%(response.status_code,content))
...
def list_pods(args):
 ...
 content=response.content
 if response.status_code>=400:
  raise xbJGW('Unable to fetch list of pods from API (code %s): %s'%(response.status_code,content))
 ...
 local_pods=CloudPodManager.load_config(safe=xbJGe).get('pods',{})
 local_pods=[{'pod_name':k,'state':'Local Only',**v}for k,v in local_pods.items()]
 existing_names=xbJGQ([pod['pod_name']for pod in result])
 result.extend([pod for pod in local_pods if pod['pod_name']not in existing_names])
 return result
def _pod_config(pod_name,config=xbJGw):
 config=config or CloudPodManager.load_config()
 pod_config=config.get('pods',{}).get(pod_name)
 if not pod_config:
  raise xbJGW('Unable to find config for pod named "%s"'%pod_name)
 return pod_config
