...
class RawDataApi(metaclass=Singleton):
    ...
    def get_oversea_fund_ret_with_benchmark(self, start_date: str = '', end_date: str = '', fund_id: str=''):
        with RawDatabaseConnector().managed_session() as db_session:
            try:
                query = db_session.query(
                    OSFundNav
                ...
                if df.empty:
                    ...
                    return result
                last_unit_nav = df.nav.values[-1] / df.nav.values[0]
        ...
        with RawDatabaseConnector().managed_session() as db_session:
            try:
                ...
                for asset, asset_list in asset_dic.items():
                    ...
                    func = fund_dic[asset]
                    _df = func(asset_list)
                    _dic_e = _df.set_index('codes').to_dict()['asset_name_e']
                    _dic_c = _df.set_index('codes').to_dict()['asset_name_c']
                    dic_c.update(_dic_c)
                    dic_e.update(_dic_e)
                df_ch = df.replace(dic_c)
                df_en = df.replace(dic_e)
                res = {'df_ch':df_ch,'df_en':df_en}
                return res
            except Exception as e:
                print('Failed to get data <err_msg> {}'.format(e))
                return None
    ...
    def get_fund_hold_equity_info(self, asset_ids:Tuple[str] = ()):
        ...
        with RawDatabaseConnector().managed_session() as db_session:
            try:
                ...
                df = pd.read_sql(query.statement, query.session.bind)
                df = df.rename(columns={'stock_name_c':'asset_name_c','stock_name_e':'asset_name_e'})
                dic = {'CH':'A股','HK':'港股','US':'美股'}
                codes = df.codes.tolist()
                c_name = df.asset_name_c.tolist()
                df.asset_name_c = [j+' '+_parse(i.split(' ')[1]) for i,j in zip(codes,c_name)]
                return df
            except Exception as e:
                print('Failed to get data <err_msg> {}'.format(e))
                return None
    ...
    def get_fund_hold_corp_info(self, asset_ids:Tuple[str] = ()):
        with RawDatabaseConnector().managed_session() as db_session:
            try:
                ...
                df = pd.read_sql(query.statement, query.session.bind)
                df = df.rename(columns={'bond_name_e':'asset_name_e','company_name_c':'asset_name_c'})
                df.asset_name_c = df.asset_name_c.map(lambda x: x +'債券' if '債' not in x else x)
                return df
            except Exception as e:
                print('Failed to get data <err_msg> {}'.format(e))
                return None
    ...
    def get_fund_hold_govt_info(self, asset_ids:Tuple[str] = ()):
        with RawDatabaseConnector().managed_session() as db_session:
            try:
                ...
                df = pd.read_sql(query.statement, query.session.bind)
                df = df.rename(columns={'bond_name_e':'asset_name_e','bond_name_c':'asset_name_c'})
                return df
            except Exception as e:
                print('Failed to get data <err_msg> {}'.format(e))
                return None
    ...
    def get_fund_hold_mtge_info(self, asset_ids:Tuple[str] = ()):
        with RawDatabaseConnector().managed_session() as db_session:
            try:
                ...
                df = pd.read_sql(query.statement, query.session.bind).rename(columns={'bond_name_e':'asset_name_e'})
                df.loc[:,'asset_name_c'] = None
                return df
            except Exception as e:
                print('Failed to get data <err_msg> {}'.format(e))
                return None
