...
def featureExpressionSig(cluster1, projectionName, featureNumber, alpha, mu):
...
def projectionDriver(patterns_filtered, cluster1, cluster2, alpha, varName, featureNumber):
    ...
    standardBon = BonferroniCorrectedDifferenceMeans(cluster1, cluster2, alpha, varName)
    driverBon = pd.DataFrame(np.transpose(plusminus), index=cluster1.var[varName], columns=['Low', 'High'])
    commonGenes = set(standardBon.index).intersection(set(driverBon.index))
