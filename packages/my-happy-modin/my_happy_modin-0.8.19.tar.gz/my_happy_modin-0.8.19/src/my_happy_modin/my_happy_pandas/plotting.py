...
class Plotting(object):
    ...
    def __getattribute__(self, item):
        """Convert any my_happy_modin DataFrames in parameters to pandas so that they can be plotted normally."""
        if hasattr(pdplot, item):
            func = getattr(pdplot, item)
            if callable(func):

                def wrap_func(*args, **kwargs):
                    """Convert my_happy_modin DataFrames to pandas then call the function."""
                    args = tuple(
