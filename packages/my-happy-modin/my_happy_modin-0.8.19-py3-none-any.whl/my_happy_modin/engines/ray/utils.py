...
from my_happy_modin.config import (
    IsRayCluster,
    RayRedisAddress,
    CpuCount,
    Memory,
...
def initialize_ray(
    ...
    override_is_cluster: bool, optional
        ...
        If not specified, $MY_HAPPY_MODIN_RAY_CLUSTER env variable is used.
    override_redis_address: str, optional
        What Redis address to connect to when running in Ray cluster.
        If not specified, $MY_HAPPY_MODIN_REDIS_ADDRESS is used.
    ...
    if threading.current_thread().name == "MainThread" or override_is_cluster:
        ...
        if cluster:
            ...
            # In case anything failed above, we can still improve the memory for my_happy_modin.
            if object_store_memory is None:
                # Round down to the nearest Gigabyte.
            ...
            if global_node.head:
                ...
                ErrorMessage.single_warning(
                    f"my_happy_modin Ray engine was started with {ray_free_space_GB} GB free space avaliable, "
                    "if it is not enough for your application, please set environment variable "
                    "MY_HAPPY_MODIN_ON_RAY_PLASMA_DIR=/directory/without/space/limiting"
                )
