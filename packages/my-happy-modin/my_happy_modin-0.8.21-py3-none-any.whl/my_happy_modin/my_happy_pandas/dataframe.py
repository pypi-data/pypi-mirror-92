...
from my_happy_modin.utils import _inherit_docstrings, to_pandas, hashable
...
class DataFrame(BasePandasDataset):
    ...
    def append(self, other, ignore_index=False, verify_integrity=False, sort=False):
        if sort is False:
            warnings.warn(
                ...
                "Pandas ignores sort=False; my_happy_modin correctly does not sort."
            )
    ...
    def info(
        ...
        if null_counts and verbose:
            ...
            # works kinda slow with `my_happy_modin.Series`, that's why we call `_to_pandas()` here
            # that will be faster.
...
if IsExperimental.get():
    from my_happy_modin.experimental.cloud.meta_magic import make_wrapped_class
