...
class ExcelDispatcher(TextFileDispatcher):
    ...
    def _read(cls, io, **kwargs):
        if (
            ...
            warnings.warn(
                "my_happy_modin only implements parallel `read_excel` with `openpyxl` engine, "
                'please specify `engine=None` or `engine="openpyxl"` to '
                "use my_happy_modin's parallel implementation."
            )
        ...
        from my_happy_modin.backends.pandas.parsers import PandasExcelParser
        ...
        warnings.warn(
            ...
            "bug_reports@my_happy_modin.org if you run into any problems."
        )
