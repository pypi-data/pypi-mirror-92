...
async def api_caller(prompt, image, forward):
    global endpoint
    port = 8080
...
async def clip_main(prompts, images, forward):
    ...
    for prompt in prompts:
        tasks[prompt] = {}
        for image in images:
            tasks[prompt][image] = asyncio.create_task(api_caller(prompt, image, forward))
