...
def test_so_tasks_filter():
    ...
    tasks_df = pd.DataFrame(
        data={
            "id": ["1", "2"],
            "so_id": ["1", None],
            "created_at": [now_datetime, now_datetime],
            "updated_at": [now_datetime, now_datetime],
            "due_date": [now_datetime, now_datetime],
        }
    )
    result_should_be = pd.DataFrame(
        data={"id": ["1"], "so_id": ["1"], "created_at": now_date, "updated_at": now_date, "due_date": now_date,}
    )
