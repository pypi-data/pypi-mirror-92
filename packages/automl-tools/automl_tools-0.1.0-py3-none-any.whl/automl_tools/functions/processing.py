...
from automl_tools.functions.utils import target_count_class, iqr_outliers, zr_score_outlier, winsorization_outliers, get_tabulate
...
def get_dataset_info(train, test, target):
    ...
    model_type, count_class = target_count_class(train[target])
...
def get_dataset_statistical(train, test, target):
    ...
    for col in train.select_dtypes('number').columns:
        ...
        elif 2 < train[col].value_counts().shape[0] <= 50:
            numeric_continuous_columns.append(col)
        elif int(train[col].value_counts().shape[0]) == int(total_rows):
    ...
    drop_all_columns = missing_columns + id_columns + date_columns + [target]
...
def get_dataset_outlier_processing(train, test):
    ...
    for col in train.select_dtypes('number').columns:
        ...
        if is_out == "yes":
            ...
            first_percetnile, ninetynine_percentile = np.percentile(train[col], [1, 99])
            ...
            train[col].loc[train[col] < lower_bound] = first_percetnile
            test[col].loc[test[col] < lower_bound] = first_percetnile
