...
# 扫描元素并执行-点击事件
def t_click(name: str = 'null', value: str = 'null', driver: str = 'null'):
    """
    [GUI元素扫描并点击事件]\n
    ...
    """
    try:
        if name == 'null' or value == 'null' or driver == 'null':
            return print("入参异常：查询类，具体值，driver值 不能为空...")
        print('click - type==%s ，value==%s ，time==%s' % (name, value, datetime.datetime.now()))
        if name == 'id':
            ui.WebDriverWait(driver, 10).until(lambda w: driver.find_element_by_id(value))  # 10秒内每隔500毫秒扫描1次页面变化
            driver.find_element_by_id(value).click()  # 执行操作
        if name == 'name':
            ui.WebDriverWait(driver, 10).until(lambda w: driver.find_element_by_name(value))  # 10秒内每隔500毫秒扫描1次页面变化
        ...
        if name == 'xpath':
            ui.WebDriverWait(driver, 10).until(lambda w: driver.find_element_by_xpath(value))  # 10秒内每隔500毫秒扫描1次页面变化
...
def t_clear(name: str = 'null', value: str = 'null', driver: str = 'null'):
    """
    [GUI元素扫描并点击事件]\n
    :param name: str 要查询的类，如ID/NAME/CLASS/XPATH；
    :param value: str 具体的元素值；
...
def t_send_keys(name: str = 'null', value: str = 'null', key: str = 'null', driver: str = 'null'):
    """
    [GUI元素扫描并点击事件]\n
    :param name: str 要查询的类，如ID/NAME/CLASS/XPATH；
    :param value: str 具体的元素值；
    ...
    """
    try:
        if name == 'null' or value == 'null' or key == 'null' or driver == 'null':
            return print("入参异常：查询类，具体值，driver值 不能为空...")
...
# 设定元素扫描(单数)
def wait_one(name: str = 'null', value: str = 'null', driver: str = 'null'):
    """
    [GUI元素扫描并点击事件]\n
    :param name: str 要查询的类，如ID/NAME/CLASS/XPATH；
    :param value: str 具体的元素值；
