...
class makeup(object):
    ...
    def apply_eyeshadow(self, landmarks_array, R, G, B):
        ...
        val = color.rgb2lab(
            (int(im[self.x, self.y]/255.)).reshape(len(self.x), 1, 3)).reshape(len(self.x), 3)
        ...
        image_blank[self.x, self.y] = color.lab2rgb(
            val.reshape(len(self.x), 1, 3)).reshape(len(self.x), 3)*255
        print("plouc19")
        original = color.rgb2lab(
            (im[self.x, self.y]*0/255.).reshape(len(self.x), 1, 3)).reshape(len(self.x), 3)
        tobeadded = color.rgb2lab(
            (image_blank[self.x, self.y]/255.).reshape(len(self.x), 1, 3)).reshape(len(self.x), 3)
        original += tobeadded
        im[self.x, self.y] = color.lab2rgb(original.reshape(
