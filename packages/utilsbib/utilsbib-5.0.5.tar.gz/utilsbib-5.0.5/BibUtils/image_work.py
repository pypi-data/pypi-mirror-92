...
def add_makeup_PIL(file,face_cropper,m_type=0, col = None):
    ...
    if m_type == 0: #lipstick
        img = PIL.Image.fromarray(np.uint8(makeup_saloon.apply_lipstick2(landmarks,[col[0],col[1],col[2]])))
    if m_type == 1: #blush
        img = PIL.Image.fromarray(np.uint8(makeup_saloon.apply_blush(landmarks,col[0],col[1],col[2] )))  
    if m_type == 2 : # eye shadow
        print("ziak 1")
        img = PIL.Image.fromarray(np.uint8(makeup_saloon.apply_eyeshadow(landmarks,col[0],col[1],col[2] )))  
        print("ziak 2")
    img_cv = np.array(img)
...
def original_and_makeup_slices(filename,faceCropper,nb_tiles,makeup_type, makeup_color):
    ...
    if makeup_type == 0: #Lipstick
        mouth_original, m_o_shape, origine = fc.isolate_mouth("", preImg=pp[0])#.convert("RGB")
        mouth_target, m_t_shape,origine2 = fc.isolate_mouth("", preImg=pp[1],pre_shape=m_o_shape)
        m_o_slices = fc.slice_img("",number_tiles=nb_tiles, pre_img=mouth_original)
        m_t_slices = fc.slice_img("",number_tiles=nb_tiles, pre_img=mouth_target)
        return m_o_slices, m_t_slices, origine
    if makeup_type == 1: #blush
        #print( "HMMMM ", pp)
        mouth_original, m_o_shape, origine = fc.isolate_cheeks("", preImg=pp[0])#.convert("RGB")
        mouth_target, m_t_shape,origine2 = fc.isolate_cheeks("", preImg=pp[1],pre_shape=m_o_shape)
        m_o_slices = fc.slice_img("",number_tiles=nb_tiles, pre_img=mouth_original)
        m_t_slices = fc.slice_img("",number_tiles=nb_tiles, pre_img=mouth_target)
        return m_o_slices, m_t_slices, origine
    if makeup_type == 2 :#eye shadow
        print( "HMMMM 2", pp)
        mouth_original, m_o_shape, origine = fc.isolate_eye("", preImg=pp[0])#.convert("RGB")
        mouth_target, m_t_shape,origine2 = fc.isolate_eye("", preImg=pp[1],pre_shape=m_o_shape)
        m_o_slices = fc.slice_img("",number_tiles=nb_tiles, pre_img=mouth_original)
        m_t_slices = fc.slice_img("",number_tiles=nb_tiles, pre_img=mouth_target)
        return m_o_slices, m_t_slices, origine
    else :
...
def original_out_slices(filename,faceCropper,nb_tiles, part_out = 0):
    ...
    if part_out == 0: # Isolate mouth without lips
         mouth_original, m_o_shape, origine = fc.isolate_mouth("", preImg=original_PIL,crop_lips=True)
    m_o_slices = fc.slice_img("",number_tiles=nb_tiles, pre_img=mouth_original)
