    ...
    def _ml_predict(self, preprocessed_x: List[Any]) -> Union[float, List[float], str, None]:
        try:
            ...
            if self.model_lib == "autogluon":
                input_data = DataFrame.from_dict(
                    {
                        name: [preprocessed_x[i]]
                        for i, name in enumerate(self.features)
                        if name not in self.non_training_features
                    }
                )
            else:
                input_data = np.array([preprocessed_x], dtype=np.float32)

            if self.predict_fn == "predict":
                y = self.models[0].predict(input_data)[0]
            else:
                y = self.models[0].predict_proba(input_data)[0]
