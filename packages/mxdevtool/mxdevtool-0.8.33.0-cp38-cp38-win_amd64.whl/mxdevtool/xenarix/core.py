from mxdevtool.mxdevtool import IndexManager_instance
...
class ScenarioBuilder:
    ...
    def checkCorrelation(self):
        m = np.matrixlib.matrix(self.corr)
        if not np.allclose(m, m.T, rtol=1e-05, atol=1e-08):
            raise Exception('correlation is not symmetric - {0}'.format(self.corr))
...
class XenarixFileManager(mx.ManagerBase):
    def __init__(self, config):
        ...
        if not os.path.exists(self.location):
            ...
            os.makedirs(os.path.join(self.location, XENREPO_RESULTS_DIR))
    ...
    def _save(self, name, ext, **kwargs):
        json_str = None
    ...
    def load_xen(self, name):
        return self._load(name, XENFILE_EXT)

    def save_xnb(self, name, **kwargs):
        for k, v in kwargs.items():
