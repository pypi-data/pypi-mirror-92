...
class DodayItem:
	...
	def __lt__(self, other):
		...
		"""
		self_cmp = str(self.order_no) if self.order_no != None else self.title
		other_cmp = str(other.order_no) if other.order_no != None else other.title
	...
	def get_options_str(self, **kwargs):
		...
		"""
		g_list = []
		str_b = self.options.get("冷熱冰量", [])
		if str_b == ["正常"]:
			str_b = ["冰"]
		g_list.extend(str_b)
		g_list.extend(self.options.get("湯底", []))
		g_list.extend(self.options.get("加料專區", []))
		g_list.extend(self.options.get("附加選項", []))
		g_list.extend(self.options.get("甜度", []))
		return ",".join(g_list)
...
class OrderInfo:
	...
	* payment_method
	...
	@classmethod
	@dutils
	def getStoreStr(cls, store_id, **kwargs):
		store_mapping = {
			"DDA": "豆日子木柵店", 
			"DDB": "豆日子辛亥店", 
			"DDC": "豆日子微風南山店"
	...
	@classmethod
	@dutils
	def getStorePhone(cls, store_id, **kwargs):
		store_mapping = {
			"DDA": "(02)86617800", 
			"DDB": "(02)86617800", 
			"DDC": "(02)86617800"
	...
	def json_format_to_order_info(cls, json_data, **kwargs):
		...
		for iidx in range(len(json_clone["name1"])):
			item_list.append(DodayItem(json_clone["name1"][iidx], json_clone["name2"][iidx], \
				json_clone["name3with4"][iidx], int(json_clone["price"][iidx]), comments=json_clone["name5"][iidx], \
				order_no=int(json_clone["order_no"][iidx])))

		json_clone.update({"current_doday_item": item_list})
	...
	def check_valid_json_format(cls, json_data, **kwargs):
		...
		order_length_flag = all(x == len(json_data["name1"]) for x in (len(json_data["name2"]), \
			len(json_data["name3with4"]), len(json_data["name5"]), len(json_data["price"]), len(json_data["order_no"])))

		key_exist_flag = set(["order_id", "store_id", "stayortogo", "payment_method"]).issubset(set(json_data.keys()))
	...
	def tojson(self, **kwargs):
		...
		json_raw_data = {
			...
			"name3with4": [item.options for item in self.orders],
	...
	def toprinterargs(self, **kwargs):
		...
		# serial number
		print_order_args.append(c_char_p(self["serial_number"].encode('utf-8')))
	...
	def get_pos_info(self, **kwargs):
		...
		c_list = ["塑膠袋: 無" if self.get_plastic_bag_str() == "" else self.get_stayortogo_str()]
		...
		return {
			...
			"order_id_label": self["serial_number"],
			"phone_number": self["phone_number"],
			"comment_list": c_list
		}
		

	def recursive_change_page_helper(cvs, start_height, prod_list, price_list):
		"""
		...
		for p in range(len(prod_list)):
			if start_height-p*20-changeline_offset < 120:
				...
				recursive_change_page_helper(cvs, 720, prod_list[p:], price_list[p:])
				break
	...
	def to_invoice_pdf(self, current_time):
		...
		for idv_order in self:
			product_str = idv_order["name2"] + " - "
			addon_dict = idv_order["name3with4"]
		...
		recursive_change_page_helper(cvs, 620, product_list, price_list)
	...
	def get_total_price(self, **kwargs):
		...
		for item in self.orders:
			tprice += item.price
		return tprice-self["total_discount"]
