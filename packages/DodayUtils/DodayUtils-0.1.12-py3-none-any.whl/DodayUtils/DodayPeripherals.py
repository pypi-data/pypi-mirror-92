	...
	def print_order_sticker(self, order_info, **kwargs):
		...
		for i in range(len(order_info)):
			...
			sf = StickerFormat(default_font_path=self.default_font_path, 
				left_header="編號%s"%str(order_info["serial_number"]), right_header="%02d/%02d"%(i, len(order_info)))
			
			# Handle special day
			if order_info.orders[i]["title"] == "special_day":
				title_str = order_info.orders[i]["title"] + "@ "+order_info.orders[i]["name3with4"]["基底"]
			else:
			...
			print("storeId: ", order_info["store_id"])
