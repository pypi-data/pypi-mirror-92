...
def d2250print(func):
	...
	def handle_d2250_print(*args, **kwargs):
		...
		args[0].send_to_d2250(tmp_fp, job_name)
...
class StickerFormat:
	def __init__(self, sticker_lines=[], **kwargs):
		...
		self.sticker_lines = sticker_lines

		self.edge_space = kwargs.get("edge_space", 10)
		self.small_font = self.get_d2250_font(22)
		self.default_font = self.get_d2250_font(34)
		self.big_font = self.get_d2250_font(39)
...
class D2250_sticker:
	...
	def send_to_d2250(self, tmp_folder_path, job_name):
		...
		conn.printFile('ARGOX_D2-250_PPLB',tmp_folder_path+"/output_sticker.pdf",job_name,{})
	...
	@d2250print
	def print_order_sticker(self, order_info):
		...
		for i in range(len(order_info)):
			...
			sf = StickerFormat(default_font_path=self.default_font_path, 
				left_header="編號%s"%order_info["serial_number"], right_header="%02d/%02d"%(i, len(order_info)))
			
			# Handle special day
			if order_info[i]["title"] == "special_day":
				title_str = order_info[i]["title"] + "@ "+order_info[i]["name3with4"]["基底"]
			else:
				title_str = order_info[i]["title"]
			sf.append({"string": "{} - ${}".format(title_str,str(order_info[i]["final_price"])), "font": "big_font"})
			sf.add_repeat_char(" ", "small_font")

			sf.append({"string": "選配: "+order_info[i].get_options_str(), "font": "default_font"}, delimiter=',')
			sf.append({"string": sf.spread_string(OrderInfo.getStoreStr(order_info["store_id"]), \
				OrderInfo.getStorePhone(order_info["store_id"]), sf.small_font),"font": "small_font"}, end_header=True)
	...
	@d2250print
	def print_inventory_sticker(self, inventory_item):
	...
	@d2250print
	def print_food_sticker(self, food_item):
...
class DodayPeripherals:
	...
	def get_qr_scan_data_com(self, tout, **kwargs):
		...
		while True:
			...
			self.doday_lib.minc_read_qrcode(c_int(1), qrcode_str)
