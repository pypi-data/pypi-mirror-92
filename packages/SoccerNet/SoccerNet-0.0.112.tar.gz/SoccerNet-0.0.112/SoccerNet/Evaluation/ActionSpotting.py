...
def evaluate(SoccerNet_path, Predictions_path, prediction_file="results_spotting.json", split="test", framerate=2):
    ...
    for game in tqdm(list_games):
        ...
        if zipfile.is_zipfile(SoccerNet_path):
            labels = LoadJsonFromZip(SoccerNet_path, os.path.join(game, "Labels-v2.json"))
        else:
            labels = json.load(open(os.path.join(SoccerNet_path, game, "Labels-v2.json")))
        # convert labels to vector
        label_half_1, label_half_2 = label2vector(labels)
        ...
        predictions_half_1, predictions_half_2 = predictions2vector(predictions)
...
def label2vector(labels, num_classes=17, framerate=2):
...
def predictions2json(predictions_half_1, predictions_half_2, output_path, game_info, prediction_file="results_spotting.json", framerate=2):

    os.makedirs(os.path.join(output_path, game_info), exist_ok=True)
    output_file_path = os.path.join(output_path, game_info, prediction_file)
...
def predictions2vector(predictions,num_classes=17, framerate=2):
