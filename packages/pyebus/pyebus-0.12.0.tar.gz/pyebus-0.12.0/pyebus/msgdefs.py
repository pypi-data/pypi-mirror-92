...
class MsgDefs:
    ...
    def find(self, circuit, name="*"):
        ...
        msgdefs = MsgDefs()
        for msgdef in self:
            if fnmatch(msgdef.circuit, circuit) and fnmatch(msgdef.name, name):
                msgdefs.add(msgdef)
        return msgdefs
        ...
        if mat:
            ...
            for msgdef in self.find(circuit, name):
                if fieldname is None:
                    ...
                    fields = tuple(fielddef for fielddef in msgdef.children if fnmatch(fielddef.name, fieldname))
                if not fields:
                    continue
