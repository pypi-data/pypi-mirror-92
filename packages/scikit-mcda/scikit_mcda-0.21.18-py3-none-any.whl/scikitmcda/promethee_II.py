...
class PROMETHEE_II(MCDA):
    ...
    def __flows(self):
        df_temp = pd.DataFrame({"alternatives":  list(self.df_original.iloc[:, 0]),
                                "Preference Flow +":  list(self.df_apf.iloc[:, 1:].sum(axis=1)/(self.df_apf.shape[0]-1)),
                                "Preference Flow -": list(self.df_apf.iloc[:, 1:].sum(axis=0)/(self.df_apf.shape[0]-1))
                                })
        
        df_temp["Net Flow"] = df_temp.iloc[:, 1] - df_temp.iloc[:, 2] 
    ...
    def __dif_pref(self):
        ...
        n_colsrows = len(self.df_original.iloc[:,0].tolist())
        x = 0
        for i in range(n_colsrows):
             for j in range(n_colsrows):            
                if i != j: 
                    df_apf.iloc[i, j] = df_temp.iloc[x, 4]
                    x = x + 1
    ...
    def __promethee_ii(self):
        ...
        df_ranking = self.df_flows.sort_values(by=["Net Flow"], ascending=False)
        df_ranking["rank"] = i
