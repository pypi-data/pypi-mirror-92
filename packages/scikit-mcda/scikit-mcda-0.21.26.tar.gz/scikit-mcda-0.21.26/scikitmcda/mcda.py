...
class MCDA:
    ...
    def dataframe(self, alt_data, alt_labels=[], criteria_labels=[]):
        """
        ...
        if criteria_labels == []:
            for s in range(0, len(alt_data[0])):
                criteria_labels.append("C" + str(s+1))
        ...
        df_data.insert(loc=0,
                    column='alternatives',
                    value=alt_labels)
        dfo = copy.deepcopy(df_data)

        self.df_original = copy.deepcopy(dfo)
        self.df_calc = copy.deepcopy(dfo)
        self.decision = []
