...
def iq_get_candles(iq,symbol,timeframe):
    ...
    return  iq.get_candles(symbol, timeframe*60, 1000, time.time())
...
def iq_get_all_candles(iq,symbol,timeframe,start_candle):
    ...
    for x in range(1):
        iq_login(iq = iq, checkConnection = True)
        data = iq.get_candles(symbol, timeframe*60, 1000, start_candle)
        start_candle = data[0]['to']-1
        final_data.extend(data)
    return final_data

def iq_get_data_needed(iq,symbol,symbols,timeframe): 
    start_candle = time.time()
    actives = symbols
    final_data = pd.DataFrame()
    for active in actives:
        current = iq_get_all_candles(iq,symbol,timeframe*60,start_candle)
        main = pd.DataFrame()
...
def iq_fast_data(iq,symbol,timeframe): 
    iq_login(iq = iq, checkConnection = True)
    candles = iq.get_candles(symbol,60*timeframe,300,time.time())
    useful_frame = pd.DataFrame()
    main = pd.DataFrame()
