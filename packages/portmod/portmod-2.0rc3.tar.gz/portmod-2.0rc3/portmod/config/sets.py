...
def get_set(package_set: str, parent_dir: Optional[str] = None) -> Set[Atom]:
    ...
    elif package_set == "selected-sets":
        set_file = os.path.join(env.prefix().PORTMOD_LOCAL_DIR, "world_sets")
    elif package_set in ("rebuild", "modules"):
        set_file = os.path.join(env.prefix().PORTMOD_LOCAL_DIR, package_set)
    else:
        set_file = os.path.join(parent_dir, package_set)
