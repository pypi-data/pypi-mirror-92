...
def scan_file(filename: str, repo_root: str, err: Callable[[str], None]):
    ...
    if ext.lower() == ".pybuild":
        ...
        if relative == os.path.join("profiles", "arch.list"):
            ...
            if len(os.path.split(relative)) == 2:
...
def main():
    ...
    commit_msg_parser = subparsers.add_parser(
        "commit-msg",
        help="Produces a commit message using the working directory. Designed to be used as a git commit-msg hook",
        parents=[common],
    )
    commit_msg_parser.add_argument("initial_message", nargs="?")
