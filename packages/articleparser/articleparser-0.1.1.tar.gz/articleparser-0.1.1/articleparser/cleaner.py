...
class Cleaner:
    ...
    # def remove_breaks(self) -> None: # deprecated
    # """Remove line break tags <br> from HTML.

    # If <br/> happens within a paragraph, insert a newline
    # in its position. Otherwise just delete it.
    ...
    #     # if <br> is adjacent to non-whitespace text, replace with newline
    #         # tag.previous_sibling and tag.next_sibling return None if the
    #         # tag is the first or last child respectively
    #     if (
    #         self._is_nonwhitespace_nstring(tag.previous_sibling)
    #         or self._is_nonwhitespace_nstring(tag.next_sibling)
    #     ):
    #         tag.replace_with("\n")
    #     # if <br> is not adjacent to non-whitespace text, remove
    #     else:
    #         tag.decompose()
    # return
    ...
    def clean(self) -> None:
        ...
        decompose_tags : list[str], optional
            If `decompose` is `True`, a list of tag names to decompose.
            If `decompose` is `True` and not provided, falls back to
            `DECOMPOSE_TAGS`.
            This list should not contain "br".
        cssvis : bool, optional
            Whether to decompose tags with CSS attributes 'display:none' or
            'visibility:hidden' (default True).
        replace_breaks : bool, optional
            Whether to remove "br" tags (default True).
        unwrap_markup : bool, optional
            Whether to unwrap markup tags (default True).
        unwrap_tags : list[str], optional
            If `unwrap_markup` is True, a list of tag names to unwrap.
            This list should be chosen from tags in `PHRASING_TAGS`.
            This list should not contain "a".
            If `unwrap_markup` is `True` and not provided, falls back to
            `MARKUP_TAGS`.
        get_linkdensity : bool, optional
