...
def filt(files, command_base, b_d_base, threads, mag_dict, length_dict, ref_file):
	...
	for sample in cleaned_bams:
		...
		md_files.append([command_base[1] + "/MetaPop/02.Filtered_Samples/"+base_name+"_sorted.bam", base_name, ref_file, command_base[1]])
...
def sort_read(commands):

	time_format = "%d/%m/%Y %H:%M:%S"
	
	timer = datetime.now()
	printable_time = timer.strftime(time_format)
	print("Sorting:", commands[0], "starting at:", printable_time)
	
	pysam.sort("-o", commands[1], commands[0])
	pysam.index(commands[1])
	
	timer = datetime.now()
	printable_time = timer.strftime(time_format)
	print(commands[0], "finished at:", printable_time)
...
def fill_mdz(args):
	...
	calmd = ["samtools", "calmd", "-b", file, ref]
	
	output = os.path.normpath(dir + "/MetaPop/02.Filtered_Samples/"+base_name+"_filled_MD.bam")
	
	filled_md = open(output, "wb")
	#We want the user to see changed MD:Z: so we don't suppress this stderr.
	subprocess.call(calmd, stdout = filled_md)
	filled_md.close()
