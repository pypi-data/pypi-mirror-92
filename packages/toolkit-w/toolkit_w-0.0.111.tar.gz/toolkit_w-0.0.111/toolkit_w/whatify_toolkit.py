...
class MyWhatify:
    ...
    def set_demo_site_api(cls):
        ...
        '''
        cls.set_magento_api(magento2url='http://whatify.tempurl.co.il/whatify/api/db/',
    ...
    def magento_api(cls, api_method: str, body: str = None, table: str = None, page: str = None,
        ...
        try:
            ...
            logging.info('Failed to get data from api, got error: ' + str(e))
        if ret_json:
            try:
                retval = pd.DataFrame.from_dict(ret_json['data']['rows'], orient='columns')
            except Exception as ex:
                logging.info('Failed to convert data into DataFrame, got error: ' + str(ex))
        return retval
