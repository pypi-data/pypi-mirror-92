...
class MyWhatify:
    ...
    def magento_api(cls, api_method: str, body: str = None, table: str = None, page: str = None,
                    limit: str = None, addtotal: str = None, onlytotal: str = None, get_df: bool=True):
        '''
        ...
        base_url = base_url[:-1]
        try:
            retval = cls.base_api(url=base_url, method=api_method, headers=cls.magento_headers, body=body)
        except Exception as e:
            logging.info('Failed to get data from api, got error: ' + str(e))
        if retval and get_df:
            try:
                retval = pd.DataFrame.from_dict(retval['data']['rows'], orient='columns')
            except Exception as ex:
