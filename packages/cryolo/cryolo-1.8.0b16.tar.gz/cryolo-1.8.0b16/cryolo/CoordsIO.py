...
def read_eman1_helicon(path, min_distance=0):
    ...
    if os.stat(path).st_size != 0:
        ...
        for filament_index, filament in enumerate(coord_filaments):
            ...
            f.add_box(BoundBox(int(first_and_last_coord[0] - boxsize / 2), int(first_and_last_coord[1] - boxsize / 2), boxsize, boxsize))
            ...
            for coords in filament:
                if len(coords)>0:
                    ...
                    #    f.add_box(candbox)
                    f.add_box(candbox)
            f.add_box(BoundBox(int(first_and_last_coord[2] - boxsize / 2), int(first_and_last_coord[3]- boxsize / 2), boxsize,
                               boxsize))
            if min_distance>0:
