    ...
    if filament_mode:
        if predict_3D:
            ...
            if  tomo_search_range == -1:
                ...
                tomo_search_range = np.mean(boxsize)/2

            if tomo_memory == -1:
                tomo_memory = max(1, np.mean(boxsize) // 10)

            if tomo_filament_window_size == -1:
                tomo_filament_window_size = int(np.mean(boxsize))*3
                ...
                if is_3d == False:
                    ...
                    if "filaments_traced" in result:
                        ...
                        for fil_id, fil in enumerate(result["filaments_traced"]):
                            xs = [box.x for box in fil_boxes]
                            fil_boxes.extend(fil.boxes)
                            fil_ids.extend([fil_id]*len(fil.boxes))
                        ...
                        np.savetxt(coords_fid_traced, np.array(fil_ids), fmt='%f')
