...
def dir_path_for_uploading(dir: Path, ext: str) -> Tuple[str, str]:
    """Create a file name for the upload that includes the full directory
    ...
    """
    dir_name = quote(str(dir)).strip("/").replace("/", "-")
    unique_uuid = str(uuid4())
    return unique_uuid, str(Path(unique_uuid) / f"{dir_name}.{ext}")
