...
class Layer(Function): #used in case that the layer have weights 
    """
    ...
    def _update_weights(self, lr,i,j,layer_type):
        ...
        if i==0:
            ...
            np.savetxt(layer_type+" Layer"+ str(j) + ".csv", weiiights, delimiter=",")
...
class Conv2D(Layer):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0,mode=0,layerNo=1):
        super().__init__()
        ...
        self._init_weights(in_channels, out_channels, self.kernel_size,mode,layerNo)

    def _init_weights(self, in_channels, out_channels, kernel_size,mode,layerNo):
        if mode==0:
        ...
        if (mode==1):   
            data = pd.read_csv("Conv_Layer"+str(layerNo)+".csv",low_memory=False ,header=None)
            data = np.asarray(data)
            self.weight['W'] = data[0:data.shape[0]-1,:]
            self.weight['b'] = data[data.shape[0]-1:data.shape[0],:]
