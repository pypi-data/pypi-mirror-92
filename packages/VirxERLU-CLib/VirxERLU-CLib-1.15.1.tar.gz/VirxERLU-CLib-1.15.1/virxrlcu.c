	...
	if(post_info.fits)
	{
		...
		if (do_ground_shot || do_jump_shot)
		{
            ...
            if (in_2d_field(&ball_offset, &me->hitbox))
            {
                ...
                double distance_remaining = flat_dist(final_target, me->location) - (me->hitbox.length * 0.95);
                Vector final_direction = normalize(sub(final_target, me->location));
		...
		if (!r.found && (do_double_jump_shot || do_aerial_shot))
		{
            ...
            if (!r.found && in_field(&ball_offset, &me->hitbox) && find_slope(shot_vector, car_to_ball) > aerial_shot_min_slope)
            {
