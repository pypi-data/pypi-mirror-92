...
Vector get_final_target(Vector *ball_offset, Vector *shot_vector, Vector *car_to_ball, Car *me, double *T)
{
    ...
    double adjustment = angle2D(car_to_offset_target, *shot_vector) * cap(*T, 0.5, 3);
...
_Bool can_reach_target_forwards(double max_time, double jump_time, double boost_accel, double *distance_remaining, double *car_speed, unsigned char car_boost, double total_jump_accel)
{
    double v = *car_speed;
    double t = 0;
    ...
    const double T = max_time + simulation_dt;
    const double double_dt = simulation_dt * 2;

    while (t <= T && (v <= 0 || d / v > jump_time))
    {
    ...
    const double top_speed_after_jump = max_speed - (total_jump_accel / 10);

    if (v > top_speed_after_jump)
        v = top_speed_after_jump;
    ...
    return d <= fabs(v) * simulation_dt * 3;
}
...
_Bool legacy_generic_is_viable(double *T, double jump_time, double *boost_accel, Vector *car_forward, double *car_speed, unsigned char *car_boost, Vector direction, double *distance_remaining, double *total_jump_accel)
{
    ...
    {
        double turn_rad = turn_radius(abs_car_speed);
        forward_time -= forward_angle * turn_rad / abs_car_speed;
        backward_time -= backward_angle * turn_rad / abs_car_speed;
    }
    else
    {
    ...
    _Bool forward = forward_time > 0 && can_reach_target_forwards(forward_time, jump_time, *boost_accel, distance_remaining, car_speed, *car_boost, *total_jump_accel);
    _Bool backward = backward_time > 0 && forward_angle >= 1.6 && *car_speed <= max_speed_no_boost && can_reach_target_backwards(backward_time, jump_time, distance_remaining, car_speed);
...
_Bool generic_is_viable(double *T, double jump_time, double *boost_accel, Car *me, Vector direction, double *distance_remaining, double *total_jump_accel)
{
    ...
    if (car_speed != 0)
    {
        ...
        forward_time -= forward_angle * turn_rad / car_speed;
        backward_time -= backward_angle * turn_rad / car_speed;
    }
    else
    {
        forward_time -= 2 * (forward_angle / PI);
    ...
    _Bool forward = forward_time > 0 && can_reach_target_forwards(forward_time, jump_time, *boost_accel, distance_remaining, &true_car_speed, me->boost, *total_jump_accel);
    true_car_speed *= -1;
    _Bool backward = backward_time > 0 && forward_angle >= 1.6 && true_car_speed < 1000 && can_reach_target_backwards(backward_time, jump_time, distance_remaining, &true_car_speed);
...
_Bool ground_shot_is_viable(double *T, double ball_radius, double *boost_accel, Car *me, double offset_target_z, Vector *direction, double *distance_remaining, double slope)
{
    double total_jump_accel = 0;
    double slope_jump_time = 0.05;

    if (offset_target_z >= (ball_radius + (me->hitbox.height / 2)) || me->airborne || slope < get_adjusted_min_slope(T, &slope_jump_time, &ground_shot_min_slope))
        return 0;
...
_Bool jump_shot_is_viable(double *T, double ball_radius, double *boost_accel, Vector *gravity, Car *me, double offset_target_z, Vector *direction, double *distance_remaining, double slope)
{
    JumpTime jump_info = get_jump_time(offset_target_z, me->velocity.z, gravity->z);

    if (*T - jump_info.time <= 0 || offset_target_z < ball_radius + (me->hitbox.height / 2) || offset_target_z > max_jump_height + (me->hitbox.height / 2) || me->airborne || slope < get_adjusted_min_slope(T, &jump_info.time, &jump_shot_min_slope))
        return 0;
...
_Bool legacy_double_jump_shot_is_viable(double *T, double *boost_accel, double *distance_remaining, Vector *direction, Vector *car_forward, unsigned char *car_boost, double *car_speed, double slope)
{
    ...
    return forward_time > 0 && can_reach_target_forwards(forward_time, jump_time, *boost_accel, distance_remaining, car_speed, *car_boost, total_jump_accel);
}
...
Shot parse_slice_for_shot_with_target(_Bool do_ground_shot, _Bool do_jump_shot, _Bool do_double_jump_shot, _Bool do_aerial_shot, double *time_remaining, double *ball_radius, double *boost_accel, Vector *gravity, Ball *ball_slice, Car *me, Target *targets)
{
	...
	if(post_info.fits)
	{
		...
		{
            Vector shot_vector = clamp2D(direction, normalize(sub(r.targets.left, ball_slice->location)), normalize(sub(r.targets.right, ball_slice->location)));
            Vector ball_offset = get_ball_offset(ball_slice, &shot_vector, ball_radius);
			double slope = find_slope(flatten(shot_vector), flatten(car_to_ball));
            ...
            {
                Vector final_target = get_final_target(&ball_offset, &shot_vector, &car_to_ball, me, time_remaining);
                double distance_remaining = flat_dist(final_target, me->location) - (me->hitbox.length * 0.45);
                Vector final_direction = normalize(sub(final_target, me->location));

                if (do_ground_shot)
                {
                    r.found = ground_shot_is_viable(time_remaining, *ball_radius, boost_accel, me, ball_offset.z, &final_direction, &distance_remaining, slope);
                    r.shot_type = GROUND_SHOT;
                }

                if (do_jump_shot && !r.found)
                {
                    r.found = jump_shot_is_viable(time_remaining, *ball_radius, boost_accel, gravity, me, ball_offset.z, &final_direction, &distance_remaining, slope);
		...
		if (do_aerial_shot && !r.found)
		{
            ...
            Vector ball_offset = get_ball_offset(ball_slice, &shot_vector, ball_radius);
            if (in_field(&ball_offset, &me->hitbox) && find_slope(shot_vector, car_to_ball) > aerial_shot_min_slope)
            {
...
Shot parse_slice_for_shot(_Bool do_ground_shot, _Bool do_jump_shot, _Bool do_double_jump_shot, _Bool do_aerial_shot, double *time_remaining, double *ball_radius, double *boost_accel, Vector *gravity, Ball *ball_slice, Car *me)
{
	...
	if ((do_ground_shot || do_jump_shot || do_double_jump_shot) && in_2d_field(&ball_offset, &me->hitbox))
	{
		...
		if (do_ground_shot)
		{
            r.found = ground_shot_is_viable(time_remaining, *ball_radius, boost_accel, me, ball_offset.z, &offset_direction, &distance_remaining, max_shot_0_slope);
        	r.shot_type = GROUND_SHOT;
		}

		if (do_jump_shot && !r.found)
		{
            r.found = jump_shot_is_viable(time_remaining, *ball_radius, boost_accel, gravity, me, ball_offset.z, &offset_direction, &distance_remaining, max_shot_0_slope);
        	r.shot_type = JUMP_SHOT;
		}
...
static PyObject *method_ground_shot_is_viable(PyObject *self, PyObject *args)
{
    ...
    {
        shot_viable = ground_shot_is_viable(&T, 92.75, &boost_accel, &me, offset_target_z, &direction, &distance_remaining, max_shot_0_slope);
    }
...
static PyObject *method_jump_shot_is_viable(PyObject *self, PyObject *args)
{
    ...
    {
        shot_viable = jump_shot_is_viable(&T, 92.75, &boost_accel, &gravity, &me, offset_target_z, &direction, &distance_remaining, max_shot_0_slope);
    }
