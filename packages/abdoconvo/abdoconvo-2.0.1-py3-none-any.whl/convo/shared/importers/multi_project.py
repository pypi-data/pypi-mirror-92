...
from convo.shared.utils.common import mark_as_experimental_feature
...
class MultiProjectImporter(TrainingDataImporter):
    def __init__(
        ...
        self.config = convo.shared.utils.io.read_config_file(config_file)
        if domain_path:
        ...
        extra_nlu_files = convo.shared.data.get_data_files(
            training_data_paths, convo.shared.data.is_nlu_file
        )
        extra_story_files = convo.shared.data.get_data_files(
            training_data_paths, convo.shared.data.is_story_file
        )
    ...
    def _init_from_file(self, path: Text) -> None:
        ...
        if os.path.exists(path) and convo.shared.data.is_config_file(path):
    ...
    def _init_from_directory(self, path: Text):
        for parent, _, files in os.walk(path, followlinks=True):
            for file in files:
                ...
                if convo.shared.data.is_test_stories_file(full_path):
                    self._e2e_story_paths.append(full_path)
                elif Domain.is_domain_file(full_path):
                    self._domain_paths.append(full_path)
                elif convo.shared.data.is_nlu_file(full_path):
                    self._nlu_paths.append(full_path)
                elif convo.shared.data.is_story_file(full_path):
                    self._story_paths.append(full_path)
                elif convo.shared.data.is_config_file(full_path):
    ...
    def _is_in_imported_paths(self, path) -> bool:
        return any(
            [convo.shared.utils.io.is_subdirectory(path, i) for i in self._imports]
        )
