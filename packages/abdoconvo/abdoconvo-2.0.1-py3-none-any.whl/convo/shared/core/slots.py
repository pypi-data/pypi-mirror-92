...
from convo.shared.constants import DOCS_URL_SLOTS

logger = logging.getLogger(__name__)


class InvalidSlotTypeException(ConvoException):
...
class Slot:
    ...
    def resolve_by_type(type_name) -> Type["Slot"]:
        ...
        for cls in convo.shared.utils.common.all_subclasses(Slot):
            if cls.type_name == type_name:
                return cls
        try:
            return convo.shared.utils.common.class_from_module_path(type_name)
    ...
    def persistence_info(self) -> Dict[str, Any]:
        return {
            "type": convo.shared.utils.common.module_path_from_instance(self),
...
class CategoricalSlot(Slot):
    ...
    def add_default_value(self) -> None:
        ...
        if convo.shared.core.constants.DEFAULT_CATEGORICAL_SLOT_VALUE not in values:
            self.values.append(
                convo.shared.core.constants.DEFAULT_CATEGORICAL_SLOT_VALUE
            )
