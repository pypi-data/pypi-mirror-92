...
from convo.shared.core.events import ActionExecutionRejected
...
from convo.core.policies.policy import Policy, SupportedData
...
class PolicyEnsemble:
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        if training_trackers:
            ...
            self.action_fingerprints = convo.core.training.training.create_action_fingerprints(
    ...
    def _persist_metadata(self, path: Text) -> None:
        ...
        policy_names = [
            convo.shared.utils.common.module_path_from_instance(p) for p in self.policies
        ]
    ...
    def load_metadata(cls, path) -> Any:
        ...
        metadata = json.loads(
            convo.shared.utils.io.read_file(os.path.abspath(metadata_path))
        )
        return metadata
    ...
    def load(cls, path: Union[Text, Path]) -> "PolicyEnsemble":
        ...
        ensemble_cls = convo.shared.utils.common.class_from_module_path(
...
class SimplePolicyEnsemble(PolicyEnsemble):
    ...
    def _get_prediction(
        ...
        number_of_arguments_in_convo_1_0 = 2
        arguments = convo.shared.utils.common.arguments_of(
            policy.predict_action_probabilities
        )
        if (
            len(arguments) > number_of_arguments_in_convo_1_0
...
class InvalidPolicyConfig(ConvoException):
