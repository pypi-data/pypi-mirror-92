...
def sort_labels(labels):
    ...
    classes = set(labels)
    ok_labels = sorted({l for l in labels if isinstance(l, int) or PATTERN_OK in l.lower()}, reverse=INCLASS_REVERSE)
...
################################################################
def make_labels(root, hrchy='label', extensions='all'):
    '''
    root 내 파일들의 filename, filepath 및 하위 디렉토리 이름을 데이터프레임으로 반환
    (예) 아래 구조의 hrchy는 'channel/label'임
      /dataset
          ├ CH1
          │   ├ NG
          │   └ OK
    ...
    if type(extensions) is not list:
        if extensions in EXTENSIONS.keys():
            exts=EXTENSIONS['all']
        else:
            exts = [extensions]
    
    files = list_files(root, exts, posix=True)
