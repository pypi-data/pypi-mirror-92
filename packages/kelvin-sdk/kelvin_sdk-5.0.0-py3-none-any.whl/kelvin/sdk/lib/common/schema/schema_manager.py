...
from kelvin.sdk.lib.common.exceptions import ContentDoesNotMatchSchemaException, InvalidApplicationConfiguration
...
def generate_base_schema_template(app_creation_parameters_object: AppCreationParametersObject) -> dict:
    ...
    if app_creation_parameters_object.app_type == AppType.kelvin:
        ...
        interface_block = {"type": app_creation_parameters_object.kelvin_app_interface.value}
        interface_block.update(app_creation_parameters_object.kelvin_app_interface.default_template)
...
def validate_app_schema_from_app_config_file(
    ...
    spec_version: str = str(KelvinAppConfiguration(**app_config_content).spec_version)
    ...
    latest_schema = _get_and_persist_app_schema(
        schema_url=SchemaManagerConfigs.specific_app_schema_url,
...
def get_latest_app_schema_version(overwrite_schema: bool = True) -> Tuple[str, Optional[dict]]:
    ...
    general_schema = _get_and_persist_app_schema(
        schema_url=SchemaManagerConfigs.general_app_schema_url,
        schema_version="app",
        schema_storage_path=schema_storage_path,
        overwrite_schema=overwrite_schema,
    )
    latest_version = _get_latest_version_from_schema(schema=general_schema)

    if not latest_version:
        message = f"No version available for the provided url: {SchemaManagerConfigs.general_app_schema_url}"
        raise InvalidApplicationConfiguration(message)
...
def _get_and_persist_app_schema(
    ...
    try:
        ...
        logger.error(f"Error retrieving the latest app configuration schema: {str(exc)}")
        return None


def _get_latest_version_from_schema(schema: Optional[dict]) -> Optional[str]:
    """
    Return the latest schema version from the provided schema.

    :param schema: the schema to retrieve the version from.

    :return: the corresponding latest version of the schema.

    """
    schema = schema or {}
    versions = schema.get("properties", {}).get("spec_version", {}).get("enum", [])
