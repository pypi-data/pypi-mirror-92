...
class KSDKHelpMessages:
    ...
    yes: str = "If specified, will ignore the destructive warning of the operation and proceed regardless."
    ...
    current_session: str = "The platform where kelvin-sdk is currently logged into."
    ...
    reset: str = "If specified will reset the configuration files required by KSDK, leading to a clear session."
    token_full: str = "Return the full authentication_token, not just the access authentication_token field."
    token_margin: str = """
        Minimum time to expiry (in seconds) for authentication_token (new authentication_token retrieved if previous authentication_token expires within margin).
        \b
    ...
    autocomplete_message: str = """\
        ...
        \b
            Bash:
                ...
                $ {{command}} configuration autocomplete --shell bash > ~/.bashrc.ksdk
                $ echo "source ~/.bashrc.ksdk" >> ~/.bashrc

        \b
            ZSH:

                $ {{command}} configuration autocomplete --shell zsh > ~/.zshrc.ksdk
...
class GeneralConfigs:
    ...
    default_system_packages_file: str = "system_packages.txt"
    ...
    default_kelvin_app_dependency: str = "kelvin-app[data]>=5.0.0"
    default_kelvin_icd_dependency: str = "kelvin-icd[builder]"
