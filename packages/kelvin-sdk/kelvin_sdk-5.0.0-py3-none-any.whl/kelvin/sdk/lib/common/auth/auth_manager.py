...
def login_on_url(
    ...
    try:
        ...
        ksdk_auth_failure: str = f"""Error authenticating: {str(exc)} \n
            Consider invalidating authentication cache with `{{command}} auth login --reset`.
        """
...
def login_client_on_current_url(
    ksdk_configuration: KelvinSDKGlobalConfiguration = None, login: bool = True, verbose: bool = True
) -> Client:
    """
...
def refresh_metadata() -> bool:
    ...
    try:
        ...
        logged_client = login_client_on_current_url()
        return bool(logged_client)
    except ConnectionError:
        logger.debug("Error retrieving metadata. Proceeding regardless..")
        return False
