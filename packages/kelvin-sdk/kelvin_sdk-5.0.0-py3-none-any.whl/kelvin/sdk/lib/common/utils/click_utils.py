...
class ClickConfigs:
    commands_with_clearance: set = {"auth", "login", "logout", "authentication_token"}
...
class KSDKGroup(Shell):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        for name in ["help", "short_help", "epilog"]:
            value = kwargs.get(name)
            if value is not None:
                kwargs[name] = value.format(command=COMMAND)
...
class KSDKCommand(click.Command):
    ...
    @staticmethod
    def commands_have_clearance(commands: list) -> bool:
        return bool(set(commands).intersection(ClickConfigs.commands_with_clearance))

    def _assess_version_status(
        self, ksdk_configuration: KelvinSDKGlobalConfiguration, protected_args: list
    ) -> VersionStatus:
        from kelvin.sdk.lib.common.utils.version_utils import assess_version_status

        # 1 - Check if commands have clearance
        if self.commands_have_clearance(commands=protected_args):
            return VersionStatus.UP_TO_DATE
        # 2 - Check the status for the provided versions
        return assess_version_status(
    ...
    def parse_args(self, ctx: Any, args: Any) -> list:
        ...
        self.version_status = self._assess_version_status(
            protected_args=ctx.protected_args, ksdk_configuration=self.ksdk_configuration
        )
    ...
    def invoke(self, ctx: Any) -> Any:
        ...
        try:
            ...
            if not self.version_status == VersionStatus.UP_TO_DATE:
                ...
                is_rc_build = check_if_is_pre_release(version=ksdk_configuration.kelvin_sdk.ksdk_latest_version)
                repository = ""
                if is_rc_build:
                ...
                if self.version_status == VersionStatus.UNSUPPORTED:
                    ksdk_version_error_warning: str = """\n
                            {red}Your version of KSDK is not supported!{reset} \n
                            {red}Current: {current_version}{reset} → {yellow}Minimum version >={minimum_version}{reset}, {green}Latest version <={latest_version}{reset} \n
                            {yellow}Changelog{reset}: https://documentation.kelvininc.com/sdk/release_notes/ \n
                            Run {green}pip3 install {repository}kelvin-sdk=={latest_version}{reset} to update\n
                            And log in again with {green}{{command}} auth login <url>{reset}.
                    """
                    unsupported_message = ksdk_version_error_warning.format_map(display_args)
                    logger.info(unsupported_message)
                elif self.version_status == VersionStatus.OUT_OF_DATE:
                    ...
                    ksdk_version_warning: str = """\n
                            ...
                            And log in again with {green}{{command}} auth login <url>{reset}.
                    """
                    out_of_date_message = ksdk_version_warning.format_map(display_args)
