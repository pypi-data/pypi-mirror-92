...
class DockerManager:
    ...
    def build_kelvin_app_dockerfile(kelvin_app_building_object: KelvinAppBuildingObject) -> bool:
        ...
        if app_language.name == "python" and app_interface.name == "client":
            dockerfile_parameters.update(
                {
                    "entry_point": language.python.entry_point,
                    "executable": interface.client.executable,
                }
            )
    ...
    def unpack_app_from_docker_image(
        ...
        app_is_extractable: bool = True
        try:
            stream, stat = self._docker_client.get_archive(container=container_object.id, path=image_dir)
        except DockerException:
            app_is_extractable = stream = False

        if app_is_extractable and stream:
