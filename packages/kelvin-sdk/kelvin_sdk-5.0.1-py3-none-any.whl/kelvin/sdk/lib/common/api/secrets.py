...
def secrets_delete(secret_name: str) -> bool:
    """
    Create a secret on the Platform.
    ...
    """
    try:
        logger.info(f'Deleting secret "{secret_name}" from the platform')

        client = login_client_on_current_url()

        prompt_question = f'This operation will delete the secret "{secret_name}" from the platform.'
