...
def convertToOBJ(dataFull,t,background=0,factor=1,Smooth=True,Decimate=True,Reduction=True,Border=2,center=[0,0,0],VoxelSize=[1,1,1],maxNumberOfThreads=40,cells_updated=None,path_write=None): ####  CONVERT SEGMENTATION IN MESH
        global _dataToConvert
        ...
        for elt in elts:
            if len(threads)<maxNumberOfThreads:
                ...
                tc=threads.pop(0)
                #print(" wait for cell " + str(tc.elt))
                tc.join()
                tc.write()
