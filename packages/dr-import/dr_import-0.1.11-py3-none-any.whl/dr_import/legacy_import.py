...
if __name__=="__main__":
  ...
  for name in contents:
    ...
    if comps[1] == ".txt":
      ...
      time = comps[0].split('-')[-1]
      video_start = datetime.datetime.strptime(time, "%y%m%d_%H%M%S_000")
  ...
  for video_time in timemap.keys():
    ...
    for video in timemap[video_time]:
      ...
      existing = api.get_media_list(project,
                                    name=safe_name,
                                    attribute=f"Trip::{args.evtr_id},Camera::{comps[1]}")
      if existing:
        ...
        with tempfile.TemporaryDirectory() as td:
          ...
          while media_id is None:
            ...
            media = tator.get_media_list(project,
                                         uid=response.uid,
                                         gid=str(gid))
            if media:
              media_id = media.id
          print(f"\r{comps[1]}:\tComplete ({media.id})")
